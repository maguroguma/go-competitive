...
func main() {
	...
	pq := NewCarrotPQ()
	for _, a := range A {
		pq.push(&Carrot{pri: int(a), length: a})
	}
	...
	for i := 0; i < plus; i++ {
		...
		var al, bl int64
		if c.length%2 == 0 {
			al, bl = c.length/2, c.length/2
		} else {
			al, bl = c.length/2+1, c.length/2
		}

		pq.push(&Carrot{pri: int(al), length: al})
		pq.push(&Carrot{pri: int(bl), length: bl})
	}
