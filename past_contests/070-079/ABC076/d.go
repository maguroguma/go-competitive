...
func main() {
	...
	velocity := 0.0
	for i := 0; i < n-1; i++ {
		var plus, stay, minus float64 // それぞれ+1で走る秒数、0で走る秒数、-1で走る秒数

		t, v := float64(T[i]), float64(V[i])
		// それぞれで区間距離を加算、velocityを更新する
		if velocity+t <= v && v <= float64(V[i+1]) { // 1:加速のみ
			ans += 0.5 * t * (velocity + t)
			velocity += t
		} else if velocity+t >= v && v <= float64(V[i+1]) { // 2:加速・stay
			ans += 0.5*(v-velocity)*((velocity)+(v)) + v*(t-(v-velocity))
			velocity = v
		} else if velocity+t >= v && v >= float64(V[i+1]) { // 3:加速・stay・減速

		} else if velocity-t <= v && v >= float64(V[i+1]) { // 4:stay・減速
