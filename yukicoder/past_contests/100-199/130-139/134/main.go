...
var dp [1 << 13][13]float64

func main() {
	xo, yo = ReadInt(), ReadInt()
	n = ReadInt()
	X, Y = make([]int, n), make([]int, n)
	W = make([]float64, n)
	for i := 0; i < n; i++ {
		x, y, w := ReadInt(), ReadInt(), ReadFloat64()
		X[i], Y[i], W[i] = x, y, w
	}

	for S := 0; S < (1 << uint(n)); S++ {
		for j := 0; j < n; j++ {
			dp[S][j] = float64(INF_BIT60)
		}
	}
	// dp[0][0] = 0.0
	...
	for S := 0; S < (1 << uint(n)); S++ {
		for j := 0; j < n; j++ {
			// 酒屋から最初の配達場所
			if S == 0 {
				dist := AbsInt(X[j]-xo) + AbsInt(Y[j]-yo)
				weight := getWeight(S)
				time := float64(dist) * ((weight + 100.0) / 120.0)

				dp[S|1<<uint(j)][j] = math.Min(dp[S|1<<uint(j)][j], time+W[j])
				continue
			}
			...
			for k := 0; k < n; k++ {
				if NthBit(S, j) == 0 && NthBit(S, k) == 1 {
					dist := AbsInt(X[k]-X[j]) + AbsInt(Y[k]-Y[j])
					weight := getWeight(S)
					time := float64(dist) * ((weight + 100.0) / 120.0)

					dp[S|1<<uint(j)][j] = math.Min(dp[S|1<<uint(j)][j], dp[S][k]+time+W[j])
				}
			}
		}
	}

	res := float64(INF_BIT60)
	for k := 0; k < n; k++ {
		res = math.Min(res, dp[1<<uint(n)-1][k]+(100.0/120.0)*float64(AbsInt(X[k]-xo)+AbsInt(Y[k]-yo)))
	}
	fmt.Println(res)
	...
	// 	fmt.Println(dp[i][:n])
	// }
	// fmt.Println(getWeight(1<<uint(n) - 1))
	// fmt.Println(getWeight(0))
}

func getWeight(SS int) float64 {
	res := 0.0
	for j := 0; j < n; j++ {
		if NthBit(SS, j) == 0 {
			res += W[j]
