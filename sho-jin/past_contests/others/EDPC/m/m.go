...
var dp [100 + 5][100000 + 5]int
var sums [100 + 5][100000 + 5]int
...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j <= k; j++ {
			...
			dp[i+1][j] %= MOD
			if j > A[i] {
				dp[i+1][j] = NegativeMod(dp[i+1][j]-sums[i][j-A[i]], MOD)
			}
		}
	}

	// for i := 0; i <= n; i++ {
	// 	fmt.Println(PrintIntsLine(dp[i][:k+1]...))
	// }
	// fmt.Println("---")
	// for i := 0; i <= n; i++ {
	// 	fmt.Println(PrintIntsLine(sums[i][:k+1]...))
	// }
