...
func solve() int {
	...
	for i := 0; i < n; i++ {
		T[B[i]] = i // T[x] は x が本来いるべき場所を記憶する
	}
	// Aのすべての要素について、自身が含まれるループを調べながらチェックしていく
	for i := 0; i < n; i++ {
		// すでにループに組み込まれており、計算済みならばスキップする
		if V[i] {
			continue
		}

		cur := i  // 現在注目しているインデックス
		S := 0    // ループに含まれる重さの合計
		m := VMAX // ループ中の重さの最小値
		an := 0   // ループに含まれる要素数
		for {
		...
		ans += Min(S+(an-2)*m, m+S+(an+1)*s) // 前者が an == 1 のときにも0になるようになっている！
	}
