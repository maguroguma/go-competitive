...
func main() {
	...
	ans := MergeSort(S, n, 0, n)
	fmt.Println(ans)
}

const MAX_ARRAY_SIZE = 500000
const SENTINEL = 2000000000 // 番兵用

var L, R [MAX_ARRAY_SIZE/2 + 2]int

// [left, right) の半開区間
func MergeSort(A []int, n, left, right int) int {
	if left+1 < right {
		mid := (left + right) / 2
		v1 := MergeSort(A, n, left, mid)    // 左側ローカルの反転数
		v2 := MergeSort(A, n, mid, right)   // 右側ローカルの反転数
		v3 := merge(A, n, left, mid, right) // 左・右にまたがる（↑では数えられていない）反転数
		return v1 + v2 + v3                 // 総和が答え
	} else {
		return 0
	}
}
...
func merge(A []int, n, left, mid, right int) int {
	...
	L[n1], R[n2] = SENTINEL, SENTINEL // 番兵

	i, j := 0, 0
	for k := left; k < right; k++ {
		if L[i] <= R[j] { // ※ここの等号が安定ソートをサポートする！※
			A[k] = L[i]
			i++
		} else {
			A[k] = R[j]
