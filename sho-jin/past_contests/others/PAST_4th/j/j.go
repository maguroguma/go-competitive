...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		G[a] = append(G[a], Edge{to: b, w: Weight(c)})
		G[b] = append(G[b], Edge{to: a, w: Weight(c)})
	}
	...
	for i := 0; i < n; i++ {
		r := S[i]
		if r == 'A' {
			G[ai] = append(G[ai], Edge{to: i, w: 0})
			G[i] = append(G[i], Edge{bi, Weight(x)})
			G[i] = append(G[i], Edge{ci, Weight(y)})
		} else if r == 'B' {
			G[bi] = append(G[bi], Edge{to: i, w: 0})
			G[i] = append(G[i], Edge{ci, Weight(z)})
			G[i] = append(G[i], Edge{ai, Weight(x)})
		} else {
			G[ci] = append(G[ci], Edge{to: i, w: 0})
			G[i] = append(G[i], Edge{ai, Weight(y)})
			G[i] = append(G[i], Edge{bi, Weight(z)})
		}
	}
	...
	transit := func(cv *Vertex, AG [][]Edge) []*Vertex {
		...
		for _, e := range AG[cv.vid] {
			nv := &Vertex{vid: e.to, v: cv.v + Value(e.w)}
			res = append(res, nv)
		}
		return res
	}
	ds := NewDijkstraSolver(vinf, less, transit)
	SS := []StartPoint{{vid: 0, vzero: 0}}
	res := ds.Dijkstra(SS, n+3, G)
