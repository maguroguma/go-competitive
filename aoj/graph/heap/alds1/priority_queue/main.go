...
func maxHeapify(i int) {
	...
	// maxヒープ条件を満たすまで木の葉に向かって下降する
	// 自身が最大の場合は（それまでに木全体でヒープ条件を満たしているのであれば）それで十分
	if largest != i {
...
func extract() int {
	...
	// 根を取り除き、代わりに末尾の要素で根を更新し、根に対してmaxHeapify
	maxv := A[1]
	A[1] = A[n]
	n--
	maxHeapify(1)
...
func increaseKey(i, key int) {
	if key < A[i] {
		return
	}

	A[i] = key
	// 根に向かってmaxヒープ条件を満たすまで上昇する
	for i > 1 && A[i/2] < A[i] {
		A[i], A[i/2] = A[i/2], A[i]
		i /= 2
	}
