...
func solve() {
	...
	hh, _ := strconv.Atoi(strs[0])
	mm, _ := strconv.Atoi(strs[1])

	// 全部試す
	for i := 0; i < H*M; i++ {
		// 反転文字列を作る
		ans, ok := isValidTime(hh, mm)
		if ok {
			printf("%s\n", ans)
			return
		}
...
func isValidTime(hh, mm int) (string, bool) {
	hstr := []rune(strconv.Itoa(hh))
	mstr := []rune(strconv.Itoa(mm))
	if len(hstr) == 1 {
		hstr = []rune{'0', hstr[0]}
	}
	if len(mstr) == 1 {
	...
	mir := []rune{mstr[1], mstr[0], ':', hstr[1], hstr[0]}
	...
	strs := strings.Split(string(mir), ":")
	hour, _ := strconv.Atoi(strs[0])
	minute, _ := strconv.Atoi(strs[1])
	if hour < H && minute < M {
		ans := []rune{hstr[0], hstr[1], ':', mstr[0], mstr[1]}
