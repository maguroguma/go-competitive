...
func sub(cy, cx, sign, num int) (int, int) {
	if num == 0 {
		return cy, cx
	}
	...
	ans[cy][cx] = sign
	num--
	// 右、下、左、上の順に進めるかチェックする
	if cx+1 < w && ans[cy][cx+1] == -1 {
		return sub(cy, cx+1, sign, num)
	} else if cy+1 < h && ans[cy+1][cx] == -1 {
		return sub(cy+1, cx, sign, num)
	} else if cx-1 >= 0 && ans[cy][cx-1] == -1 {
		return sub(cy, cx-1, sign, num)
	} else if cy-1 >= 0 && ans[cy-1][cx] == -1 {
		return sub(cy-1, cx, sign, num)
	}
