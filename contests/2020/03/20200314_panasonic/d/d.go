...
func rec(pattern []int, num int) [][]int {
	if len(pattern) == n {
		return [][]int{pattern}
	}
	...
	for i := 0; i < num; i++ {
		...
		newPattern = append(newPattern, i)

		res = append(res, rec(newPattern, num)...)
	}
	// 新しいグループ
	newPattern := make([]int, len(pattern))
	copy(newPattern, pattern)
	newPattern = append(newPattern, num)

	res = append(res, rec(newPattern, num+1)...)

	return res
}

// FactorialPatterns returns all patterns of n! of elems([]rune).
func FactorialPatterns(elems []rune) [][]rune {
