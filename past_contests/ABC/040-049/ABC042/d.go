...
var factorials [200000 + 5]int
var invFactorials [200000 + 5]int

func main() {
	h, w, a, b = ReadInt(), ReadInt(), ReadInt(), ReadInt()

	for i := 0; i < len(factorials); i++ {
		factorials[i] = -1
		invFactorials[i] = -1
	}
	factorials[0] = 1
	invFactorials[0] = CalcModInv(factorials[0], MOD)

	for i := 1; i < len(factorials); i++ {
		factorials[i] = sub(i)
		invFactorials[i] = CalcModInv(factorials[i], MOD)
	}

	ans := 0
	for i := b; i <= w-1; i++ {
		num := 1
		num *= factorials[i+(h-1-a)]
		num %= MOD
		num *= invFactorials[i]
		num %= MOD
		num *= invFactorials[h-1-a]
		num %= MOD

		num *= factorials[(w-1-i)+(a-1)]
		num %= MOD
		num *= invFactorials[w-1-i]
		num %= MOD
		num *= invFactorials[a-1]
		num %= MOD
