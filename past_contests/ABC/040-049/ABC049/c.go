...
func main() {
	...
	reversedS := make([]rune, len(S))
	for i, r := range S {
		reversedS[len(S)-1-i] = r
	}

	i := 0
	for i < len(S) {
		if i+5 <= len(S) {
			str5 := string(reversedS[i : i+5])
			if str5 == "maerd" || str5 == "esare" {
				i += 5
				continue
			}
		}
