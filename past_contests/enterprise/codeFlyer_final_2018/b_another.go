...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		firstIdx := GeneralUpperBound(X, c-d)
		secondIdx := GeneralUpperBound(X, c)
		thirdIdx := GeneralUpperBound(X, c+d)

		ans += d * firstIdx
		ans += c*(secondIdx-firstIdx) - (sumX[secondIdx] - sumX[firstIdx])
		ans += (sumX[thirdIdx] - sumX[secondIdx]) - c*(thirdIdx-secondIdx)
		ans += d * (len(X) - thirdIdx)
