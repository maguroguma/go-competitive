...
func main() {
	...
	// for _, a := range A {
	// 	fmt.Println(string(GetZeroPaddingRuneSlice(a, 10)))
	// 	// x := 0
	// 	// for j := 0; j <= 32; j++ {
	// 	// 	if sums[i+1][j]%2 == 1 {
	// 	// 		x += (1 << uint(j))
	// 	// 	}
	// 	// }
	// 	// fmt.Println(string(GetZeroPaddingRuneSlice(x, 10)) + " <- X")
	// }
	// fmt.Println("---")
	for k := 0; k < n; k++ {
		x := 0
		...
		// fmt.Println(string(GetZeroPaddingRuneSlice(x, 10)) + " <- X")
		// for i := 0; i <= k; i++ {
		// 	fmt.Println(string(GetZeroPaddingRuneSlice(A[i]^x, 10)))
		// }

		ans := 0
		for j := 0; j <= 32; j++ {
			if GetNthBit(x, j) == 1 {
				ans += (1 << uint(j)) * ((k + 1) - sums[k+1][j])
			} else {
				ans += (1 << uint(j)) * sums[k+1][j]
			}
		}
