...
func main() {
	...
	for i := 0; i < h; i++ {
		...
		if !ok {
			rowIdxList = append(rowIdxList, i)
		}
	}
	...
	for j := 0; j < w; j++ {
		...
		if !ok {
			colIdxList = append(colIdxList, j)
		}
	}

	for i := 0; i < len(rowIdxList); i++ {
		rowString := ""
		for j := 0; j < len(colIdxList); j++ {
			y, x := rowIdxList[i], colIdxList[j]
			rowString += string(A[y][x])
		}
