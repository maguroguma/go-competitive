/*
URL:
http://codeforces.com/contest/123/problem/B
*/
...
func main() {
	...
	X, Y := int64(0), int64(0)
	if (sx < 0 && gx > 0) || (sx > 0 && gx < 0) {
		X++
	}
	if (sy < 0 && gy > 0) || (sy > 0 && gy < 0) {
		Y++
	}
	sx /= b
	gx /= b
	sy /= a
	gy /= a
	ans := Max(AbsInt(sx-gx)+X, AbsInt(sy-gy)+Y)
