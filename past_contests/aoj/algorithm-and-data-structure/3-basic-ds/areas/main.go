...
type pair struct {
	// x: 水たまりの左側の座標, y: 水たまりの面積
	x, y int
}

func main() {
	S = ReadRuneSlice()
	s1 := []int{}  // 水たまりの左側「となりうる座標」を計算しておく、基本はこちらを中心に計算を進める
	s2 := []pair{} // 出来上がった水たまりの左側の座標とその面積を記憶する
	...
	for i := 0; i < len(S); i++ {
		...
		if ch == '\\' {
			...
			j := s1[len(s1)-1] // 今注目している水たまりの左座標
			s1 = s1[:len(s1)-1]
			sum += i - j // 今見ている '/' の高さ分のみの面積
			a := i - j
			// 注目している水たまりに収まる計算済みの水たまりを併合する
			for len(s2) > 0 && s2[len(s2)-1].x > j {
				a += s2[len(s2)-1].y
				s2 = s2[:len(s2)-1]
			}
			s2 = append(s2, pair{x: j, y: a})
		}
	}

	answers := []int{}
	for len(s2) > 0 {
		answers = append(answers, s2[len(s2)-1].y)
		s2 = s2[:len(s2)-1]
	}
