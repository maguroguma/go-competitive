...
func main() {
	...
	spSums, sums := make([]int, n+1), make([]int, n+1)
	// 負の数は0と見る累積和
	for i := 0; i < len(A); i++ {
		if A[i] >= 0 {
			spSums[i+1] = spSums[i] + A[i]
		} else {
			spSums[i+1] = spSums[i]
		}
	}
	...
	for l := 0; l <= n-k; l++ {
		leftSum := spSums[l]
		rightSum := spSums[n] - spSums[l+k]
		centerSum := Max(0, sums[l+k]-sums[l])

		ChMax(&ans, leftSum+centerSum+rightSum)
	}
