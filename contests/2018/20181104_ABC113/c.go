...
func main() {
	...
	prefs := [100005][]int{}
	for i := 0; i < m; i++ {
		prefs[P[i]] = append(prefs[P[i]], Y[i])
	}
	for i := 0; i <= n; i++ {
		sort.Sort(sort.IntSlice(prefs[i]))
	}
	orderMemo := make(map[string]int)
	for i := 0; i <= n; i++ {
		for j, year := range prefs[i] {
			str := strconv.Itoa(i) + strconv.Itoa(year)
			orderMemo[str] = j + 1
		}
	}
	for i := 0; i < m; i++ {
		//		var order int
		//		for j, year := range prefs[P[i]] {
		//			if year == Y[i] {
		//				order = j + 1
		//				break
		//			}
		//		}
		//		// orderとP[i]から認識番号を出力
		//		fmt.Println(sub(P[i], order))
		key := strconv.Itoa(P[i]) + strconv.Itoa(Y[i])
		fmt.Println(sub(P[i], orderMemo[key]))
	}
}

func sub(pi, order int) string {
	var piStr, orderStr string
	picopy := pi
	ordercopy := order
	// piの桁数チェック
	digitNum := 1
	...
	piStr = strconv.Itoa(pi)
	for i := 0; i < 6-digitNum; i++ {
		piStr = "0" + piStr
	}
	// orderStr
	digitNum = 1
