...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		X = append(X, x)
		Y = append(Y, y)
		xarray = append(xarray, node{id: i, coord: x})
		yarray = append(yarray, node{id: i, coord: y})
	}
	...
	sort.Sort(nodeSlice(xarray))
	sort.Sort(nodeSlice(yarray))

	for i := 1; i < len(xarray); i++ {
		cost := xarray[i].coord - xarray[i-1].coord
		idx1, idx2 := xarray[i].id, xarray[i-1].id
		edges[idx1][idx2] = cost
		edges[idx2][idx1] = cost
	}
	for i := 1; i < len(yarray); i++ {
		cost := yarray[i].coord - yarray[i-1].coord
		idx1, idx2 := yarray[i].id, yarray[i-1].id
		if _, isSet := edges[idx1][idx2]; isSet {
			edges[idx1][idx2] = Min(edges[idx1][idx2], cost)
		} else {
			edges[idx1][idx2] = cost
		}
		if _, isSet := edges[idx2][idx1]; isSet {
			edges[idx2][idx1] = Min(edges[idx2][idx1], cost)
		} else {
			edges[idx2][idx1] = cost
		}
	}
