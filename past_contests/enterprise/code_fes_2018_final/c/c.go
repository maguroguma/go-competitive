...
func main() {
	...
	L = make(PlanList, 0, 200000+5)
	for i := 0; i < n; i++ {
		L = append(L, &Plan{key: A[i], a: A[i], b: B[i]})
	}
	sort.Stable(byKey{L})
...
func sub(t int) int {
	...
	res := 0
	if ok == -1 {
		res = Min(L[0].b + Max(0, t - L[0].a), L[1].b + Max(0, t - L[1].a))
	} else if ok == len(L)-1 {
		res = L[ok].b + Max(0, t - L[ok].a)
	} else {
		res = Min(L[ok].b+Max(0, t-L[ok].a), L[ok+1].b+Max(0, t-L[ok+1].a))
	}
...
func (l PlanList) Swap(i, j int) {
	l[i], l[j] = l[j], l[i]
}

func (l byKey) Less(i, j int) bool {
	return l.PlanList[i].key < l.PlanList[j].key
}

// how to use
// L := make(PlanList, 0, 200000+5)
// L = append(L, &Plan{key: intValue})
