...
func main() {
	...
	om, em = make(map[int]int), make(map[int]int)
	for i := 0; i < n-1; i++ {
		d := H[i+1] - H[i]
		if i%2 == 0 {
			em[d]++
	...
	for i := 0; i < q; i++ {
		...
		if t == 1 {
			...
			if u == 0 {
				bef := (H[u+1] + V[u+1]) - (H[u] + V[u])
				aft := (H[u+1] + V[u+1]) - (H[u] + V[u] + v)
				em[bef]--
				em[aft]++
			} else if u == n-1 {
				bef := (H[u] + V[u]) - (H[u-1] + V[u-1])
				aft := (H[u] + V[u] + v) - (H[u-1] + V[u-1])
				if u%2 == 0 {
					om[bef]--
					om[aft]++
				} else {
