...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		rmaps[org][rates[c]]-- // レートの数をデクリメント
		if rmaps[org][rates[c]] == 0 {
			// org園内に転校する幼児のレートがいなくなったら、そのレートを削除
			rsets[org].Delete(rates[c])
		}

		// 転校（入る）
		isets[to].Insert(c) // 幼児cをtoにインサート
		if rmaps[to][rates[c]] == 0 {
			// to園内に転校してくる幼児のレートがいなかったら、そのレートを挿入
			rsets[to].Insert(rates[c])
		}
		rmaps[to][rates[c]]++ // レートの数をインクリメント
