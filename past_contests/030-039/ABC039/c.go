...
func main() {
	...
	partial := string(S[0:12])
	str := "WBWBW" + "WBWBWBW" + "WBWBW" + "WBWBWBW" + "WBWBW" + "WBWBWBW"
	memo := []rune{}
	for _, r := range str {
		memo = append(memo, r)
	}
	switch partial {
	case string(str[0:12]):
		fmt.Println("Do")
	case string(str[2:14]):
		fmt.Println("Re")
	case string(str[4:16]):
		fmt.Println("Mi")
	case string(str[5:17]):
		fmt.Println("Fa")
	case string(str[7:19]):
		fmt.Println("So")
	case string(str[9:21]):
		fmt.Println("La")
	case string(str[11:23]):
		fmt.Println("Si")
	}
}
