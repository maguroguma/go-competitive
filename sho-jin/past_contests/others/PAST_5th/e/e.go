...
// 正規化済みのはんこの底面の座標タプルのリストを返す
func getBlocks(T [][]rune) [][2]int {
	res := [][2]int{}
	for i := 0; i < len(T); i++ {
		for j := 0; j < len(T[0]); j++ {
			if T[i][j] == '#' {
...
// 総当りして可能かどうか判定する
func check(tuples [][2]int) bool {
	// 左上から全部試す
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			// tuplesを(i, j)ずらす
			ct := make([][2]int, len(tuples))
			for k := 0; k < len(ct); k++ {
				ct[k][0] = tuples[k][0] + i
				ct[k][1] = tuples[k][1] + j
			}
