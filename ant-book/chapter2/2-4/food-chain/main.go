...
func main() {
	n, k = ReadInt(), ReadInt()
	T, X, Y = make([]int, k), make([]int, k), make([]int, k)
	for i := 0; i < k; i++ {
		T[i], X[i], Y[i] = ReadInt(), ReadInt(), ReadInt()
	}

	// このUnionFind木は「各グループは、それらが全て同時に起こることがわかっていることを表す」もの
	uf := NewUnionFind(n * 3)
	ans := 0
	for i := 0; i < k; i++ {
		...
		// 以下のSameの条件部分は、いずれも最初の一行分だけで正しく動作する
		// （おそらく、Unite部分で全パターンまとめているため、それぞれの行は互いに同値なのだと思われる）
		if t == 1 {
			// 「xとyは同じ種類」という情報
			if uf.Same(x, y+n) || uf.Same(x, y+2*n) ||
				uf.Same(x+n, y+2*n) || uf.Same(x+n, y) ||
				uf.Same(x+2*n, y) || uf.Same(x+2*n, y+n) {
				ans++
			} else {
				uf.Unite(x, y)
				uf.Unite(x+n, y+n)
			...
			if uf.Same(x, y) || uf.Same(x, y+2*n) ||
				uf.Same(x+n, y+n) || uf.Same(x+n, y) ||
				uf.Same(x+2*n, y+2*n) || uf.Same(x+2*n, y+n) {
				ans++
			} else {
				uf.Unite(x, y+n)
				uf.Unite(x+n, y+2*n)
