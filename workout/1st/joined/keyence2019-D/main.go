...
func main() {
	...
	comp1, _ := RunLengthEncoding(A)
	comp2, _ := RunLengthEncoding(B)
	if len(comp1) != n || len(comp2) != m {
		fmt.Println(0)
		return
	}
	...
	for i := n * m; i >= 1; i-- {
		...
		if h != -1 && w != -1 {
			...
			col := BinarySearch(-1, len(B), func(mid int) bool {
				return B[mid] > i
			}) + 1
			...
			row := BinarySearch(-1, len(A), func(mid int) bool {
				return A[mid] > i
			}) + 1
