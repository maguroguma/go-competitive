...
func main() {
	...
	tmp := ReadIntSlice(a)
	S = append(S, -PowInt(10, 12))
	S = append(S, tmp...)
	S = append(S, PowInt(10, 12))
	tmp = ReadIntSlice(b)
	T = append(T, -PowInt(10, 12))
	T = append(T, tmp...)
	T = append(T, PowInt(10, 12))
	X = ReadIntSlice(q)
	...
	for i := 0; i < len(X); i++ {
		...
		// 左右の最寄りの神社・寺を見つける
		sl, sr, tl, tr := -1, -1, -1, -1
		sn := GeneralUpperBound(S, x)
		tn := GeneralLowerBound(T, x)
		sl, sr, tl, tr = sn-1, sn, tn-1, tn
		for _, s := range []int{sl, sr} {
			for _, t := range []int{tl, tr} {
				spos, tpos := S[s], T[t]
				d1 := AbsInt(x-spos) + AbsInt(spos-tpos)
				d2 := AbsInt(x-tpos) + AbsInt(tpos-spos)
				ChMin(&ans, d1)
				ChMin(&ans, d2)
			}
		}
