...
func main() {
	...
	// PrintfDebug("%v\n", dp[:n])
	// dfs2(0, -1, 0)
	// fmt.Println(PrintIntsLine(answers[:n]...))
}

// func dfs2(cid, pid, dpar int) {
// 	dchilds := []int{}
// 	nexts := []int{}
...
// 	}
// 	other := 0
// 	for _, v := range dchilds {
// 		other += Max(v, 0)
// 	}
// 	other += Max(dpar, 0)
// 	answers[cid] = root + other
...
// 	}
// 	if l == 1 {
// 		dfs2(nexts[0], cid, root+Max(dpar, 0))
// 		return
// 	}

// 	// 子が2つ以上の場合
// 	R, L := make([]int, l), make([]int, l)
// 	L[0] = Max(dchilds[0], 0)
// 	for i := 1; i < l; i++ {
// 		L[i] = L[i-1] + Max(dchilds[i], 0)
// 	}
// 	R[l-1] = Max(dchilds[l-1], 0)
// 	for i := l - 2; i >= 0; i-- {
// 		R[i] = R[i+1] + Max(dchilds[i], 0)
// 	}
// 	for i, nid := range nexts {
// 		if i == 0 {
// 			dfs2(nid, cid, root+R[1]+Max(dpar, 0))
// 		} else if i == l-1 {
// 			dfs2(nid, cid, root+L[l-2]+Max(dpar, 0))
// 		} else {
// 			dfs2(nid, cid, root+L[i-1]+R[i+1]+Max(dpar, 0))
// 		}
// 	}
// }

// func dfs1(cid, pid int) int {
// 	res := 0
// 	if A[cid] == 0 {
// 		res--
// 	} else {
