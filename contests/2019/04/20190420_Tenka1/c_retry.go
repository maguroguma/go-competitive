...
func main() {
	...
	whiteNums := make([]int, n+1)
	blackNums := make([]int, n+1)
	for i := 0; i < n; i++ {
		if S[i] == '.' {
			whiteNums[i+1] = whiteNums[i] + 1
			blackNums[i+1] = blackNums[i]
		} else {
			whiteNums[i+1] = whiteNums[i]
			blackNums[i+1] = blackNums[i] + 1
		}
	}

	ans := math.MaxInt64
	for i := 0; i < n; i++ {
		tmp := blackNums[i+1] + whiteNums[n] - whiteNums[i+1]
		ans = Min(ans, tmp)
	}

	fmt.Println(Min(ans, whiteNums[n]))
}
