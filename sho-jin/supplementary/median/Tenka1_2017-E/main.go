...
func main() {
	...
	for i := 0; i < n; i++ {
		a, b, c := A[i], B[i], C[i]
		d := -1.0 * float64(a) / float64(b)
		e := float64(c) / float64(b)

		L = append(L, Line{d, e, i})
	}
	...
	ax := BinarySearchFloat64(INF_B60, -INF_B60, fn)

	for i := 0; i < n; i++ {
		L[i].a = 1.0 / L[i].a
		L[i].b = L[i].a * L[i].b * (-1.0)
	}
	sort.Slice(L, func(i, j int) bool {
		return L[i].a > L[j].a
	})
	...
	ay := BinarySearchFloat64(INF_B60, -INF_B60, fn)

	println(ax, ay)
}

func fn(xf float64) bool {
	Y := []YValue{}
	for i := 0; i < n; i++ {
		l := L[i]
		y := l.a*xf + l.b

		Y = append(Y, YValue{y, l.idx})
	}
	...
	num := InversionNumber(I)

	return int(num) >= Kiriage(n*(n-1), 4)
}
