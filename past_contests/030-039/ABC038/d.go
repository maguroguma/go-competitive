...
func main() {
	...
	boxList := make(MonoList, n)
	for i := 0; i < n; i++ {
		boxList[i] = &Mono{key: W[i], value: H[i]}
	}
	sort.Sort(sort.Reverse(boxList))

	newBoxList := make(MonoList, n)
	for i := 0; i < n; i++ {
		newBoxList[i] = &Mono{key: boxList[i].value, value: boxList[i].key}
	}
	sort.Sort(sort.Reverse(newBoxList))
	...
	for i := 0; i < n; i++ {
		if i == 0 {
			ans++
			beforeBox = newBoxList[0]
			continue
		}

		if beforeBox.key > newBoxList[i].key && beforeBox.value > newBoxList[i].value {
			ans++
			beforeBox = newBoxList[i]
		}
	}
