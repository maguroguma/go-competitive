...
func main() {
	...
	for i := 0; i < len(S); i++ {
		...
		if IsProductOverflow(val, cd) {
			println(0)
			return
		}
		val *= cd

		if IsSumOverflow(curV, val) {
			println(0)
			return
		}
		curV += val
	...
	// 最大のm進数を調べる
	ok := BinarySearch(md, INF_B60, func(mid int) bool {
		curV := 0
		for i := 0; i < len(S); i++ {
			cd := int(S[i] - '0')
			if cd == 0 {
