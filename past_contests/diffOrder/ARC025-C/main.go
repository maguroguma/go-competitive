...
func main() {
	...
	ans := 0
	for i := 0; i < n; i++ {
		dp, _ := dijkstra(i, n, G[:n])
		usagi, kame := []float64{}, []float64{}
		for j := 0; j < n; j++ {
			if i == j {
				continue
			}
			usagi = append(usagi, float64(dp[j])/float64(r))
			kame = append(kame, float64(dp[j])/float64(t))
		}

		sort.Slice(usagi, func(i, j int) bool { return usagi[i] < usagi[j] })
		sort.Slice(kame, func(i, j int) bool { return kame[i] < kame[j] })
		...
		for j := 0; j < n-1; j++ {
			...
			ok := BinarySearch(n-1, -1, func(mid int) bool {
				return usagi[mid] > ktime
			})
			num := (n - 1 - ok) - sub
			ans += num
		}
