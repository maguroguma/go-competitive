...
func main() {
	...
	for i := 0; i < n; i++ {
		p := P[i]
		a := Rotate90(p)
		b := Rotate90(Rotate90(a))

		v := RotateTheta(G_EPS, a)
		w := RotateTheta(-G_EPS, a)
		x := RotateTheta(G_EPS, b)
		y := RotateTheta(-G_EPS, b)
		H = append(H, v, w, x, y)
	}
	...
	println(math.Sqrt(float64(x*x + y*y)))
}

func sub(p Point) (int, []int) {
	tx, ty := 0, 0
	Q := []int{}
