...
func main() {
	...
	heap.Init(pq)
	for i := 0; i < t; i++ {
		heap.Push(pq, &Item{pri: A[i], id: i})
	}
	...
	for pq.Len() > 0 {
		...
		if answers[len(answers)-1] != fp.id {
			answers = append(answers, fp.id)
			fp.pri--
			if fp.pri > 0 {
				heap.Push(pq, fp)
		...
		if answers[len(answers)-1] == fp.id && pq.Len() > 0 {
			sp := heap.Pop(pq).(*Item)
			answers = append(answers, sp.id)
			sp.pri--
