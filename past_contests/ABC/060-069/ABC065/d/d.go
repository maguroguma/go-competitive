...
func (l CityList) Len() int {
	return len(l)
}
func (l CityList) Swap(i, j int) {
	l[i], l[j] = l[j], l[i]
}

func (l byX) Less(i, j int) bool {
	return l.CityList[i].x < l.CityList[j].x
}
func (l byY) Less(i, j int) bool {
	return l.CityList[i].y < l.CityList[j].y
}

// how to use
// L := make(CityList, 0, 200000+5)
// L = append(L, &City{key: intValue})
...
func main() {
	...
	for i := 1; i < n; i++ {
		cost := L[i].x - L[i-1].x
		edges = append(edges, &Edge{from: L[i].id, to: L[i-1].id, cost: cost})
	}
	sort.Stable(byY{L})
	for i := 1; i < n; i++ {
		cost := L[i].y - L[i-1].y
	...
	sort.Stable(byKey{edges})

	uf := NewUnionFind(100000 + 5)
	ans := 0
...
func (l byKey) Less(i, j int) bool {
	return l.EdgeList[i].cost < l.EdgeList[j].cost
}
