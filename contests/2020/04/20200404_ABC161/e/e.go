...
func (l byStart) Less(i, j int) bool {
	return l.TaskList[i].start < l.TaskList[j].start
}
func (l byEnd) Less(i, j int) bool {
	return l.TaskList[i].end < l.TaskList[j].end
}
...
func main() {
	...
	for i := 0; i < n; i++ {
		if S[i] == 'o' {
			...
			t := &Task{day: i + 1, start: start, end: end}
			L = append(L, t)
		}
	}
	...
	endTime := -INF_BIT60
	for i := 0; i < len(L); i++ {
		if endTime < L[i].start {
			forward[L[i].day] = true
			endTime = L[i].end
		}
	}
	// PrintfDebug("%v\n", forward)

	sort.Stable(byStart{L})
	startTime := INF_BIT60
	for i := len(L) - 1; i >= 0; i-- {
		if startTime > L[i].end {
			backward[L[i].day] = true
			startTime = L[i].start
		}
	}
	// PrintfDebug("%v\n", backward)
