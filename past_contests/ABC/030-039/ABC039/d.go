...
func main() {
	...
	candidate := make([][]rune, h)
	for i := 0; i < h; i++ {
		candidate[i] = make([]rune, w)
	}
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			candidate[y][x] = '#'
	...
	repaired := make([][]rune, h)
	for i := 0; i < h; i++ {
		repaired[i] = make([]rune, w)
	}
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
	...
	for i := 0; i < h; i++ {
		fmt.Println(string(candidate[i]))
	}
}
