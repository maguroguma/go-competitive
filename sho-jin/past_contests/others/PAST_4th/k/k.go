...
func main() {
	...
	for _, id := range B {
		...
		C := cnt[id]
		s := NewCumulativeSum(G[:30])
		// 数える
		for i := 0; i < 25; i++ {
			ans += s.RangeSum(i+1, 25) * C[i]
		}
...
func InversionNumber(A []int) int64 {
	var _rec func(A []int, l, r int) int64

	// _rec sorts an array A
	_rec = func(A []int, l, r int) int64 {
		if r-l == 1 {
			return 0
		}
