...
func main() {
	...
	pre, post := [2]int{}, [2]int{}
	preFlag, postFlag := false, false
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if A[i][j] == 1 {
				if !preFlag {
...
func sub(prei, prej, posti, postj int) {
	// i方向の移動を出力
	for i := prei; i < posti; i++ {
		str := fmt.Sprintf("%d %d %d %d", prei+1, prej+1, prei+1+1, prej+1)
		S = append(S, str)
		prei++
	}

	// j方向の移動を出力
	for j := prej; j < postj; j++ {
		str := fmt.Sprintf("%d %d %d %d", prei+1, prej+1, prei+1, prej+1+1)
		S = append(S, str)
		prej++
	}
}
