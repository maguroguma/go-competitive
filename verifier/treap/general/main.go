...
func main() {
	...
	segments := []T{
		T{0, 10}, T{1, 9}, T{2, 8}, T{3, 7}, T{4, 6}, T{5, 5},
		T{0, 10}, T{1, 9}, T{2, 8}, T{3, 7}, T{4, 6}, T{5, 5},
		T{0, 10}, T{1, 9}, T{2, 8}, T{3, 7}, T{4, 6}, T{5, 5},
	}
	...
	fmt.Printf("counts\n")
	for i := 0; i < 6; i++ {
		fmt.Printf("segment [%v]: %d\n", segments[i], tr.Count(segments[i]))
	}

	fmt.Printf("MinGreater\n")
	for i := 0; i < 6; i++ {
		node := tr.MinGreater(segments[i])
		if node != nil {
			fmt.Printf("segment [%v]: %v\n", segments[i], node.key)
		} else {
			fmt.Printf("segment [%v]: %v\n", segments[i], nil)
		}
	}
	...
	fmt.Printf("MinGeq\n")
	fmt.Printf("segment [%v]: %v\n", T{-100, 6}, tr.MinGeq(T{-100, 6}).key)
	fmt.Printf("MaxLeq\n")
	fmt.Printf("segment [%v]: %v\n", T{-100, 6}, tr.MaxLeq(T{-100, 6}).key)
}
