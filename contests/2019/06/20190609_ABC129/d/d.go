...
func main() {
	...
	for i := 0; i < h; i++ {
		yoko = append(yoko, make([]int, w))
		tate = append(tate, make([]int, w))
	}

	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if S[i][j] == '#' {
				yoko[i][j], tate[i][j] = -1, -1
			} else {
				yoko[i][j], tate[i][j] = 1, 1
			}
		}
	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			...
			if yoko[i][j] > 0 && tate[i][j] > 0 {
				ChMax(&ans, yoko[i][j]+tate[i][j]-1)
			} else {
