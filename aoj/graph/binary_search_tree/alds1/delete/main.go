...
func Insert(k int) {
	...
	x := root // 挿入先
	z := new(Node)
	z.key, z.left, z.right = k, nil, nil
	...
	// 挿入ノードに関して、親と挿入先を設定する（挿入先の判断にはもう一度kyeチェックが必要）
	z.parent = y
	if y == nil {
		root = z
	} else {
...
// 木全体からxの次節点を探索しそれを返す
// 次節点: xの次にkeyが大きいノード
// 木の中でxが最大のキーを持つ場合は次節点が存在しないためnilを返す
func treeSuccessor(x *Node) *Node {
	...
	// 親をたどるなかで、最初に「左の子になっているような節点の親」が次節点
	// ※通常の二分探索木では実行されることはない
	y := x.parent
	for y != nil && x == y.right {
		x = y
		y = y.parent
...
func Delete(z *Node) {
	...
	if x != nil {
		x.parent = y.parent // xの親を差し替える
	}

	if y.parent == nil {
		root = x // yが根のとき、xを木の根とする
	} else {
		if y == y.parent.left {
			y.parent.left = x // yがその親pの左の子なら、pの左の子をxに差し替える
		} else {
			y.parent.right = x // yがその親pの右の子なら、pの右の子をxに差し替える
		}
	}

	// yがzの次節点の場合は、yのkeyでzのkeyを上書きする
	if y != z {
		z.key = y.key
	}
}
