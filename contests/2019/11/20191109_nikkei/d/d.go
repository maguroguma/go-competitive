...
func (l byKey) Less(i, j int) bool {
	return l.RangeList[i].key < l.RangeList[j].key
}

// how to use
// L := make(RangeList, 0, 200000+5)
// L = append(L, &Range{key: intValue})
// sort.Stable(byKey{ L })                // Stable ASC
// sort.Stable(sort.Reverse(byKey{ L }))  // Stable DESC

func main() {
	n, m := ReadInt2()
	L, R, C = make([]int, m), make([]int, m), make([]int, m)
	for i := 0; i < m; i++ {
		l, r, c := ReadInt3()
		L[i], R[i], C[i] = l-1, r-1, c
	}

	ranges := make(RangeList, 0)
	for i := 0; i < m; i++ {
		l, r, c := L[i], R[i], C[i]
		ranges = append(ranges, &Range{key: l, l: l, r: r, c: c})
	}
	sort.Stable(byKey{ranges})

	st := NewSegTreeRMQ(n, INF_BIT60)
	st.Update(0, 0)

	for i := 0; i < len(ranges); i++ {
		l, r, c := ranges[i].l, ranges[i].r, ranges[i].c
		mini := st.Query(l, r+1)

		rdist := st.Query(r, r+1)
		if rdist > mini+c {
			st.Update(r, mini+c)
		}
	}
