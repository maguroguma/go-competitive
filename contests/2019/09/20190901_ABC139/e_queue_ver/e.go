...
func main() {
	...
	days := 0
	for len(gameQueue) > 0 {
		// その日に組まれている試合を消化する（1日経過する）
		days++

		prevGameQueue := gameQueue
		gameQueue = []Game{}
		for _, g := range prevGameQueue {
			// ここで初めて実際に試合が消化される（次の対戦相手がA[i]の先頭で参照できる）
			A[g.a] = A[g.a][1:]
			A[g.b] = A[g.b][1:]
			check(g.a)
			check(g.b)
...
// 試合の予定をqueueに組み込むイメージ
// queueに組み込まれた段階ではまだ消化されていないため、A[i]からは削除できない
// 削除できないから、次の対戦相手としてふさわしいか、都度チェックできる
func check(i int) {
	if len(A[i]) == 0 {
		return
	}
	...
	// iがjの次の対戦相手となっているかどうか
	if A[j][0] == i {
		if i > j {
			i, j = j, i
		}
		// 重複した試合がqueueに入らないようにする
		// 例えば、初試合が1->2, 2->1となっているときは、1, 2ともに同じ試合を入れようとすることになるので、これが必要
		g := Game{a: i, b: j}
		if !gameMap[g] {
			gameQueue = append(gameQueue, g)
			gameMap[g] = true
		}
	}
}
