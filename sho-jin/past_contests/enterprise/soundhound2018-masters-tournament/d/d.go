...
func main() {
	...
	iniCost := PowInt(10, 15)
	answers := make([]int, n)
	for i := 0; i < n; i++ {
		answers[i] = ydp[i] + sdp[i]
	}
	minAnswers := make([]int, n)
	curMin := INF_BIT60
	for i := n - 1; i >= 0; i-- {
		curMin = Min(curMin, answers[i])
		minAnswers[i] = curMin
	}

	for i := 0; i < n; i++ {
		fmt.Println(iniCost - minAnswers[i])
	}
}
...
func dijkstra(sid int, G [][]Edge) []int {
	dp, colors := make([]int, n), make([]int, n)
	for i := 0; i < n; i++ {
		dp[i] = INF_BIT60
		colors[i] = WHITE
	}
	...
	pq := &temp
	heap.Init(pq)
	heap.Push(pq, &Vertex{id: sid, cost: 0})
	dp[sid] = 0
	colors[sid] = GRAY
	for pq.Len() > 0 {
