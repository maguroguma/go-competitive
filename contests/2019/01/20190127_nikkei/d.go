...
func main() {
	...
	edgeList := make(EdgeList, n+m-1)
	for i := 0; i < n+m-1; i++ {
		edge := &edge{key: i, start: A[i], end: B[i]}
		edgeList[i] = edge
	}
	...
	befEnd := 0
	for i := 0; i < n+m-1; i++ {
		if befEnd == edgeList[i].end {
			continue
		} else {
			// 最初はこちらから実行される
			if befEnd-edgeList[i].end != -1 {
				//fmt.Println(0)
				newA[i], newB[i] = 0, edgeList[i].end
			}
			//fmt.Println(edgeList[i].start)
			newA[i], newB[i] = edgeList[i].start, edgeList[i].end
			befEnd = edgeList[i].end
		}
	}

	// 閉路検出
	nodeAndEdgeList := make([][]int, n)
	for i := 0; i < n; i++ {
		nodeAndEdgeList[i] = []int{}
	}
	flags := make([]bool, n)
	for i := 0; i < n; i++ {
		start, end := newA[i], newB[i]
		nodeAndEdgeList[start] = append(nodeAndEdgeList[start], end)
	}
