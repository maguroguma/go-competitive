...
var edges map[pair][]pair
var isVisited map[pair]bool

func main() {
	n, m = ReadInt(), ReadInt()

	edges = make(map[pair][]pair)
	for i := 1; i <= 999; i++ {
		for j := 1; j <= 999; j++ {
			ni, nj := i, j
			...
			p, np := pair{x: i, y: j}, pair{x: ni, y: nj}
			if _, ok := edges[p]; !ok {
				edges[p] = []pair{}
			}
			edges[p] = append(edges[p], np)
			if _, ok := edges[np]; !ok {
				edges[np] = []pair{}
			}
	...
	for i := 1; i <= n; i++ {
		for j := 1; j <= m; j++ {
			if _, ok := isVisited[pair{x: i, y: j}]; ok {
				ans++
			}
		}
	}
