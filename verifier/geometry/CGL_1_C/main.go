...
func main() {
	...
	M := map[int]string{
		G_ONLINE_FRONT:      "ONLINE_FRONT",
		G_CLOCKWISE:         "CLOCKWISE",
		G_ON_SEGMENT:        "ON_SEGMENT",
		G_COUNTER_CLOCKWISE: "COUNTER_CLOCKWISE",
		G_ONLINE_BACK:       "ONLINE_BACK",
	}

	x1, y1, x2, y2 := readf(), readf(), readf(), readf()
	a, b := NewPoint(x1, y1), NewPoint(x2, y2)
...
func Reflection(l *Line, p *Point) *Point {
	return p.Add(ProjectionToLine(l, p).Minus(p)).Mul(2.0)
}
