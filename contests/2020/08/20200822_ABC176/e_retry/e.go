...
func main() {
	...
	for i := 0; i < m; i++ {
		y, x := H[i], W[i]
		yn = append(yn, V{num: len(Y[y]), x: y})
		xn = append(xn, V{num: len(X[x]), x: x})
	}
	...
	for i := 0; i < m; i++ {
		if xn[0].num == xn[i].num {
			// xnum++
			xm[xn[i].x] = 1
		}
		if yn[0].num == yn[i].num {
			// ynum++
			ym[yn[i].x] = 1
		}
	}
	xnum, ynum = len(xm), len(ym)

	maxN := xn[0].num + yn[0].num
	pat := xnum * ynum

	ano := 0
	for i := 0; i < m; i++ {
		y, x := H[i], W[i]
		if len(Y[y])+len(X[x]) == maxN {
			ano++
		}
	}
