...
func main() {
	...
	if a*b != int(float64(a)*float64(b)) {
		fmt.Println(sub2(t, a, b))
	} else {
		fmt.Println(sub1(t, a, b))
	}
}

func sub1(t, a, b int) int {
	ab := Lcm(a, b)
...
func sub2(t, a, b int) int {
	A := float64(t) / float64(a)
	A = math.Ceil(A)
	B := float64(t) / float64(b)
	B = math.Ceil(B)
	AB := float64(t) / float64(a) / float64(b) * float64(Gcd(a, b))
	AB = math.Ceil(AB)
