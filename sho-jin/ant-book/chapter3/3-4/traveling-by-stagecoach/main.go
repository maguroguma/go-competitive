...
var n, m, a, b int
var t []int
var d [30][30]int // グラフの距離行列（辺がない場合は-1）

// dp[S][v] := 残っている乗車券がS、現在位置vという状態に至るまでの最小コスト
var dp [1 << 8][30]float64
...
func solve() {
	...
	// 初期状態ではすべてのチケットが余っていて、かつaにいる、という状態
	dp[1<<uint(n)-1][a-1] = 0

	for S := 1<<uint(n) - 1; S >= 0; S-- {
		// どのチケットを使うのか（使うためには余っていないといけない）
		for i := 0; i < n; i++ {
			if S>>uint(i)&1 == 1 {
				// 今どこにいるのか
				for v := 0; v < m; v++ {
					...
					for u := 0; u < m; u++ {
						if d[v][u] >= 0 {
							// 乗車券iを使って、vからuへ移動
							// ビット反転と論理積でフラグをおろせる（チケットが使える）
							dp[S & ^(1<<uint(i))][u] =
								math.Min(dp[S & ^(1<<uint(i))][u], dp[S][v]+float64(d[v][u])/float64(t[i]))
						}
					}
				}
			}
