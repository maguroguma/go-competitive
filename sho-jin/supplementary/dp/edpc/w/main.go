...
func main() {
	...
	n, m = readi2()
	L, R, A = make([]int, m), make([]int, m), make([]int, m)
	for i := 0; i < m; i++ {
		l, r, a := readi3()
		L[i], R[i], A[i] = l-1, r-1, a
	}
	...
	e := func() S { return -INF_B60 }
	merger := func(a, b S) S { return S(max(int(a), int(b))) }
	mapper := func(f F, x S) S { return S(int(f) + int(x)) }
	composer := func(f, g F) F { return f + g }
	id := func() F { return 0 }
	...
	for i := 0; i < m; i++ {
		l, r, a := L[i], R[i], A[i]
		segments[r] = append(segments[r], Segment{l: l, r: r, a: a})
	}
	...
	for i := 0; i < n; i++ {
		...
		for _, segm := range segments[i] {
			lseg.RangeApply(segm.l, segm.r+1, F(segm.a))
		}
	}

	ans := max(0, int(lseg.Prod(0, n)))
	println(ans)
}
