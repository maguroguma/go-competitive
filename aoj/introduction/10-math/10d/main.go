...
func main() {
	n = ReadInt()
	X, Y = ReadIntSlice(n), ReadIntSlice(n)

	for i := 1; i <= 3; i++ {
		sum := 0.0

		for j := 0; j < n; j++ {
			sum += math.Pow(math.Abs(float64(X[j]-Y[j])), float64(i))
		}
		sum = math.Pow(sum, 1.0/float64(i))
		fmt.Printf("%f\n", sum)
	}

	ans := 0.0
	for j := 0; j < n; j++ {
		if ans < math.Abs(float64(X[j])-float64(Y[j])) {
			ans = math.Abs(float64(X[j]) - float64(Y[j]))
		}
	}
	fmt.Printf("%f\n", ans)
}
