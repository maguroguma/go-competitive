...
func main() {
	...
	for _, S := range memo {
		for i := 0; i < len(S); i++ {
			if i == 0 {
				initiators = append(initiators, S[0])
				continue
			}
			followers = append(followers, S[i])
		}
	}
	sort.Sort(sort.Reverse(sort.IntSlice(initiators)))
	sort.Sort(sort.Reverse(sort.IntSlice(followers)))

	chosenFollowers := &IntHeap{}
	initFollowerNum := Min(len(followers), k)
	ans := 0
	for i := 0; i < initFollowerNum; i++ {
		heap.Push(chosenFollowers, followers[i])
		ans += followers[i]
	}
	initInitiatorsNum := k - chosenFollowers.Len()
	/* ここでは種類数を最低で見積もっている
	すなわち、ある種類についてinitiatorは選んでいないがfollowerを選んでいるというケースがあり得るが、
	そのような場合その種類の寿司は食べていないと考えて満足度を計算している*/
	for i := 0; i < initInitiatorsNum; i++ {
		ans += initiators[i]
		ans += 2*i + 1
	}

	currentAns := ans
	for i := initInitiatorsNum + 1; i <= len(memo); i++ {
		if chosenFollowers.Len() == 0 {
			break
		}
		poppedD := heap.Pop(chosenFollowers).(int)
		currentAns -= poppedD
		currentAns += initiators[i-1]
		currentAns += 2*(i-1) + 1
