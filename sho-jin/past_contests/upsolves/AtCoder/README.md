...
Last Change: 2021-04-14 00:07:59.
...
令和ABC-Bで最難、、というかC含めても相当難しい部類に入ると思う。

全探索でやり直したが、そのやり方も自分の方法はかなり賢くなかった（多分本番では自信を持って提出できない）。
公式解説の `n` 個選んだときに `a*n <= 1000*w <= b*n` の条件をチェックする、というのが最もスマートで素直。

## [ABC198 D.Send More Money](https://atcoder.jp/contests/abc198/tasks/abc198_d)

解法自体はなんてことはない順列の全探索だが、どうも自分の実装はおそすぎるようだった。

一応、メモリ確保とかは最低限で済むように書き直したが、3300msecぐらいかかってしまう。
非再帰で書かないと、速度の大幅改善はできないかもしれない。
