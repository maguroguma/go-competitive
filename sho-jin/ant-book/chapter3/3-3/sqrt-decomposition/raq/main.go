...
// [x]: https://onlinejudge.u-aizu.ac.jp/problems/DSL_2_E

const sqrtN = 512

type SqrtDecompositionRAQ struct {
	n, k            int
	data, bucketAdd []int
}

func NewSqrtDecompositionRAQ(A []int) *SqrtDecompositionRAQ {
	sd := new(SqrtDecompositionRAQ)
	sd.n = len(A)
	...
	sd.bucketAdd = make([]int, sd.k)

	return sd
}

// [s, t)
func (sd *SqrtDecompositionRAQ) Add(s, t, x int) {
	for k := 0; k < sd.k; k++ {
		...
		if s <= l && r <= t {
			...
			a, b := int(math.Max(float64(s), float64(l))), int(math.Min(float64(t), float64(r)))
			for i := a; i < b; i++ {
				sd.data[i] += x
			}
		}
...
func (sd *SqrtDecompositionRAQ) Get(i int) int {
	k := i / sqrtN
	return sd.data[i] + sd.bucketAdd[k]
}
...
func main() {
	...
	sd := NewSqrtDecompositionRAQ(make([]int, n))
	for i := 0; i < q; i++ {
		c := ReadInt()
