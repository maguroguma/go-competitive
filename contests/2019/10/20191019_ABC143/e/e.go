...
func main() {
	...
	for i := 0; i < n; i++ {
		Dijkstra(i)
		for j := 0; j < n; j++ {
			if dp[j].times < INF_30 {
				answers[i][j] = dp[j].times
			} else {
				answers[i][j] = -1
			}
		}
...
func (pq VertexPQ) Less(i, j int) bool {
	if pq[i].times < pq[j].times {
		return true
	} else if pq[i].times > pq[j].times {
		return false
	} else {
		if pq[i].gas > pq[j].gas {
			return true
		} else {
			return false
		}
...
func Dijkstra(sid int) {
	...
	for i := 0; i < v; i++ {
		// dp[i] = INF_30
		// dp[i] = Vertex{gas: 0, times: INF_30} // idはいらないはず
		dp[i] = Vertex{gas: -1, times: INF_30} // idはいらないはず
		flags[i] = false
	}
	// dp[sid] = 0
	dp[sid] = Vertex{gas: l, times: 0}
	// heap.Push(pq, &Node{pri: dp[sid], dist: dp[sid], id: sid})
	heap.Push(pq, &Vertex{gas: dp[sid].gas, times: dp[sid].times, id: sid})
	...
	for pq.Len() > 0 {
		...
		// if dp[v].times < vertex.times {
		// 	continue
		// } else if dp[v].times == vertex.times && dp[v].gas > vertex.gas {
		// 	continue
		// }
		...
		for _, e := range G[v] {
			...
			if dp[v].gas >= e.cost {
				// 補給無し
				if dp[e.to].times > dp[v].times {
					// 回数が小さいなら必ず更新
					dp[e.to].times = dp[v].times
					dp[e.to].gas = dp[v].gas - e.cost
					heap.Push(pq, &Vertex{gas: dp[e.to].gas, times: dp[e.to].times, id: e.to})
				} else if dp[e.to].times == dp[v].times && dp[e.to].gas < dp[v].gas-e.cost {
					// 回数が同じでもガソリンが多いなら更新
					dp[e.to].gas = dp[v].gas - e.cost
					heap.Push(pq, &Vertex{gas: dp[e.to].gas, times: dp[e.to].times, id: e.to})
				}
				...
				if dp[e.to].times > dp[v].times+1 {
					// 回数が小さいなら必ず更新
					dp[e.to].times = dp[v].times + 1
					dp[e.to].gas = l - e.cost
					heap.Push(pq, &Vertex{gas: dp[e.to].gas, times: dp[e.to].times, id: e.to})
				} else if dp[e.to].times == dp[v].times+1 && dp[e.to].gas < l-e.cost {
					// 回数が同じでもガソリンが多いなら更新
					dp[e.to].gas = l - e.cost
					heap.Push(pq, &Vertex{gas: dp[e.to].gas, times: dp[e.to].times, id: e.to})
				}
