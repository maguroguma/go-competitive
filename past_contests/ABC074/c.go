...
func main() {
	...
	for i := 0; i <= 30; i++ {
		for j := 0; j <= 30; j++ {
			for k := 0; k <= 100; k++ {
				for l := 0; l <= 100; l++ {
					if i*100*a+j*100*b+k*c+l*d <= f && k*c+l*d <= (i*a+j*b)*e {
						current_water := i*100*a + j*100*b
						current_sugar := k*c + l*d
						current_node := (float64(current_sugar) * 100.0) / (float64(current_water) + float64(current_sugar))
						if current_node > node {
							node = current_node
							water, sugar = current_water, current_sugar
						}
					}
				}
			}
