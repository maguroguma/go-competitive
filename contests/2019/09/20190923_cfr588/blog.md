# Codeforces Round No.588 (Div.2) 参加記録

初めてCodeforcesに参加しました。

CodeforcesはAtCoderほど日本語の解説記事が見当たらなかったので、ブログの練習も兼ねて参加録をつけていこうと思います。
...
袋はどちらか一方に所属することになるわけなので、ビット全探索を書いた。
...
ノード間の辺の有無や、ある種類のドミノの使用済みかどうかの判断はそれぞれともに行列で行っている。
...
そちらのほうが確実そうな一方で、実装方法がよくわかっていないので誰か教えてほしい。
...
- `A[i]`: このビット表現は、0から59のアルゴリズムを、i番目の生徒が知っているかどうかを示している（1ならば知っている）。
- `B[i]`: i番目の生徒のスキルの高さを示している（高ければ高いほどよい）。
