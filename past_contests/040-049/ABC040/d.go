...
var parents [100000 + 1]int
var S [100000 + 1]int

func main() {
	n, m = ReadInt(), ReadInt()
	A, B, Y = make([]int, m), make([]int, m), make([]int, m)
	edgeList := make(MonoList, m)
	for i := 0; i < m; i++ {
		a, b, y := ReadInt(), ReadInt(), ReadInt()
		A[i], B[i], Y[i] = a, b, y
		edgeList[i] = &Mono{key: i, value: y}
	}
	q = ReadInt()
	V, W = make([]int, q), make([]int, q)
	queryList := make(MonoList, q)
	for i := 0; i < q; i++ {
		v, w := ReadInt(), ReadInt()
		V[i], W[i] = v, w
		queryList[i] = &Mono{key: i, value: w}
	}
	...
	// エッジとクエリを年の降順にソートする
	sort.Sort(sort.Reverse(edgeList))
	sort.Sort(sort.Reverse(queryList))

	// エッジを新しい順に結合していく
	cfqIdx, cuqIdx := 0, 0
	ans := make([]int, q)
	for cfqIdx < q {
		queryIdx, limitYear := queryList[cfqIdx].key, queryList[cfqIdx].value

		for cuqIdx < m {
			edgeIdx, estYear := edgeList[cuqIdx].key, edgeList[cuqIdx].value

			// 結合クエリを処理する
			if estYear > limitYear {
				// fmt.Printf("Union Query: idx: %d, estYear: %d\n", edgeIdx, estYear)
				a, b := A[edgeIdx], B[edgeIdx]
				unite(a, b)
				cuqIdx++
			} else {
				break
		...
		// findクエリに回答する
		// fmt.Printf("Find Query: idx: %d, limitYear: %d\n", queryIdx, limitYear)
		ans[queryIdx] = size(V[queryIdx])
		cfqIdx++
	}
