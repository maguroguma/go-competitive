...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			...
			df := distance(x0, y0, x1, y1)

			t0, r0, t1, r1 := T[i], R[i], T[j], R[j]
			// i => j
			vi := float64(Min(t0, r1))
			G[i] = append(G[i], Edge{to: j, cost: df / vi})

			// j => i
			vj := float64(Min(t1, r0))
			G[j] = append(G[j], Edge{to: i, cost: df / vj})
		}
	}
	...
	sort.Slice(dp, func(i, j int) bool { return dp[i] > dp[j] })
	ans := 0.0
	for i := 0; i < n-1; i++ {
		plus := float64(i)
		if ans < dp[i]+plus {
...
func dijkstra(sid, n int, AG [][]Edge) (dp []float64, parents []int) {
	dp = make([]float64, n)
	parents = make([]int, n)
	colors := make([]int, n)
	for i := 0; i < n; i++ {
		dp[i] = float64(INF_DIJK)
