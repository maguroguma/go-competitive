...
func (l MountList) Len() int {
	return len(l)
}
func (l MountList) Swap(i, j int) {
	l[i], l[j] = l[j], l[i]
}

func (l byKey) Less(i, j int) bool {
	return l.MountList[i].key < l.MountList[j].key
}

// how to use
// L := make(MountList, 0, 200000+5)
// L = append(L, &Mount{key: intValue})
...
func main() {
	...
	L := make(MountList, 0)
	for i := 0; i < n; i++ {
		L = append(L, &Mount{key: H[i], id: i, height: H[i]})
	}
	sort.Stable(sort.Reverse(byKey{L}))

	// for i := 0; i < n; i++ {
	// 	PrintfDebug("id: %d, height: %d\n", L[i].id, L[i].height)
	// }
	...
	for i := 0; i < n; i++ {
		// 処理前にフラッシュが必要
		if i > 0 && L[i-1].height > L[i].height {
			// tmpの中身をフラッシュする
			for j := 0; j < len(tmp); j++ {
				tr.Insert(tmp[j])
			}
		...
		left := tr.MaxLess(mount.id)
		right := tr.MinGreater(mount.id)

		num := 0
		if left == nil {
			num += mount.id
		} else {
			num += mount.id - left.key - 1
		}
		if right == nil {
			num += n - mount.id - 1
		} else {
			num += right.key - mount.id - 1
		}

		answers[mount.id] = num
		tmp = append(tmp, mount.id)
