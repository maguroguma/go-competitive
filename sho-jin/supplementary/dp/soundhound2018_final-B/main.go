...
// https://jupiro.hatenablog.com/entry/2020/08/14/174328
func main() {
	defer stdout.Flush()
	...
	for i := 0; i < n; i++ {
		...
		chmax(&dp[i+1], dp[i]+B[i])
		if i+1-k >= 0 {
			chmax(&dp[i+1], cm[i+1-k])
		}
		chmax(&cm[i+1], dp[i+1])
	}
...
func NewCumSum(A []int) *CumSum {
	cs := new(CumSum)

	cs.csum = make([]int, len(A)+1)
	for i := 0; i < len(A); i++ {
...
func (cs *CumSum) Sum(l, r int) int {
