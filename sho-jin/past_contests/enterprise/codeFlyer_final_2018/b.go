...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		firstIdx := GeneralLowerBound(X, c-d) - 1
		secondIdx := GeneralLowerBound(X, c) - 1
		thirdIdx := GeneralLowerBound(X, c+d) - 1

		ans += d * (firstIdx + 1)

		num := secondIdx - firstIdx
		partialSum = 0
		if firstIdx >= 0 {
			partialSum -= sumX[firstIdx]
		}
		if secondIdx >= 0 {
			partialSum += sumX[secondIdx]
		}
		ans += c*num - partialSum

		num = thirdIdx - secondIdx
		partialSum = 0
		if secondIdx >= 0 {
			partialSum -= sumX[secondIdx]
		}
		if thirdIdx >= 0 {
			partialSum += sumX[thirdIdx]
		}
		ans += partialSum - c*num

		ans += d * (len(X) - 1 - thirdIdx)
