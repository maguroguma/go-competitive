...
- `dp[i]: i番目の箱を最も外側にしたときの最大ネスト数` としてメモ化再帰で解く。
  - メモ化再帰を普段書かないので練習目的で書いてみた。
  - 初期化と遷移の仕方には注意（適当にやりすぎて最初間違えた）
- ソートも特別な構造体も不要で、計算量も `O(N^2)` とはっきりわかるいい解き方。
  - こういった考え方もできるようにしておきたい。
