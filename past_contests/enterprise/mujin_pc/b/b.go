...
func main() {
	...
	for i := 0; i < (1 << 3); i++ {
		...
		ChMax(&maxi, AbsInt(sum))
		ChMin(&mini, AbsInt(sum))
	}

	maxF, minF := float64(maxi), float64(mini)
	var ans float64
	...
	if isTriangle {
		ans = math.Pi * maxF * maxF
	} else {
		ans = math.Pi*maxF*maxF - math.Pi*minF*minF
	}
