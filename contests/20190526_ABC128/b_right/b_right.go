...
func (l byName) Less(i, j int) bool {
	return l.ItemList[i].name < l.ItemList[j].name
}
func (l byScore) Less(i, j int) bool {
	return l.ItemList[i].score < l.ItemList[j].score
}
...
func main() {
	n = ReadInt()
	L := make(ItemList, 0, 100000)
	for i := 0; i < n; i++ {
		S, p := string(ReadRuneSlice()), ReadInt()
		L = append(L, &Item{
			key:   i,
			idx:   i,
			name:  S,
	...
	// sort.Sort(sort.Reverse(byScore{ItemList(L)}))
	// sort.Sort(byName{ItemList(L)})
	sort.Stable(sort.Reverse(byScore{ItemList(L)}))
	sort.Stable(byName{ItemList(L)})
	for i := 0; i < len(L); i++ {
		fmt.Println(L[i].idx + 1)
	}
}
