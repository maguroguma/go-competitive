...
func main() {
	...
	ans := BinarySearch(0, INF_BIT60, func(k int) bool {
		if r-k < 0 || b-k < 0 {
			return false
		}
		...
		// return FloorFrac(r-k, x-1)+FloorFrac(b-k, y-1) >= k

		// return int(math.Floor(float64(r-k)/float64(x-1)))+int(math.Floor(float64(b-k)/float64(y-1))) >= k
	})

	PrintfDebug("(-3)/5: %d\n", (-3)/5)
	PrintfDebug("math.Floor: (-3)/5: %d\n", int(math.Floor(float64(-3)/float64(5))))
	PrintfDebug("original: (-3)/5: %d\n", FloorFrac(-3, 5))
