...
func main() {
	first, last = ReadString(), ReadString()
	n = ReadInt()
	for i := 0; i < n; i++ {
		str := ReadString()
	...
	for i := 0; i < wn; i++ {
		for j := i + 1; j < wn; j++ {
			s, t := words[i], words[j]
			if wordDiff(s, t) == 1 {
				G[i] = append(G[i], Edge{to: j, cost: 1})
				G[j] = append(G[j], Edge{to: i, cost: 1})
			}
		}
	}

	dp, _ := dijkstra(sid, wn, G)
	if dp[gid] >= INF_DIJK {
		fmt.Println(-1)
		return
	}
	...
	answers := []string{}
	cid := gid
	answers = append(answers, words[cid])
	for cid != sid {
		for _, e := range G[cid] {
			if dp[e.to]+e.cost == dp[cid] {
				cid = e.to
				answers = append(answers, words[cid])
