...
var n, m int
var A, B, C []int
var adjMatrix [16][16]int

var dp [1 << 16][16]int
var flags [1 << 16][16]bool
...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		ChMax(&adjMatrix[A[i]][B[i]], C[i])
		ChMax(&adjMatrix[B[i]][A[i]], C[i])
	}

	for S := 0; S < (1 << uint(n)); S++ {
		for j := 0; j < n; j++ {
			if S == 0 {
				flags[S|(1<<uint(j))][j] = true
				continue
			}

			for k := 0; k < n; k++ {
				if NthBit(S, j) == 0 && NthBit(S, k) == 1 && adjMatrix[k][j] > 0 && flags[S][k] {
					dist := adjMatrix[k][j]
					ChMax(&dp[S|(1<<uint(j))][j], dp[S][k]+dist)
					flags[S|(1<<uint(j))][j] = true
				}
			}
		}
