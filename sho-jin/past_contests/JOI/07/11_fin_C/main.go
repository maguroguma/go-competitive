...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		// G[a] = append(G[a], Edge{to: c, w: Weight{hl}})
		// G[c] = append(G[c], Edge{to: a, w: Weight{hl}})
		// G[b] = append(G[b], Edge{to: c, w: Weight{hl}})
		// G[c] = append(G[c], Edge{to: b, w: Weight{hl}})

		G[a] = append(G[a], Edge{to: b, w: Weight{l}})
		G[b] = append(G[b], Edge{to: a, w: Weight{l}})
	...
	estimate := func(cid int, cv Value, e Edge) Value {
		return Value{cv.v + e.w.w}
	}
	SS := []StartPoint{}
	for i := 0; i < k; i++ {
		SS = append(SS, StartPoint{id: S[i], vzero: Value{0}})
	}
	N := n
	AG := G[:N]

	ds := NewDijkstraSolver(vinf, less, estimate)
	dp := ds.Dijkstra(SS, N, AG)
	debugf("dp: %v\n", dp)
	...
	for a := 0; a < n; a++ {
		for _, e := range G[a] {
			b := e.to
			lf := float64(dp[a].v+dp[b].v+e.w.w) / 2.0
			if ans < lf {
				ans = lf
			}
		}
