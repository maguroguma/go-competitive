...
func main() {
	...
	// fmt.Println(ones, fives)

	// fmt.Println(combination(A, k))

	patterns := combination(A, k)
	ans := 1 << 18
...
func combination(input []int, k int) [][]int {
	copiedResidual := make([]int, len(input))
	copy(copiedResidual, input)
	return comb([]int{}, copiedResidual, k)
}

func comb(interim, residual []int, k int) [][]int {
	if len(interim) == k {
		return [][]int{interim}
	}
	...
	for idx, elem := range residual {
		...
		copiedInterim = append(copiedInterim, elem)
		// copiedResidual := genDeletedSlice(idx, residual)
		copiedResidual := subdel(idx, residual)
		res = append(res, comb(copiedInterim, copiedResidual, k)...)
	}

	return res
}

func subdel(delId int, S []int) []int {
	res := []int{}
	// res = append(res, S[:delId]...)
	res = append(res, S[delId+1:]...)
	return res
