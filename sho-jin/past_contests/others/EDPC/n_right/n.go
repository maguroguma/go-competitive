...
var dp [400 + 5][400 + 5]int
var flags [400 + 5][400 + 5]bool
var sums [400 + 5]int
...
func sub(start, goal int) int {
	...
	flags[start][goal] = true

	if start == goal {
		dp[start][start] = 0 // 必須！！！（flagを立てたあとはdp[start][start]を見に行くため）
		return 0
	}

	for i := start; i <= goal-1; i++ {
		ChMin(&dp[start][goal], sub(start, i)+sub(i+1, goal))
	}
	dp[start][goal] += (sums[goal+1] - sums[start])
