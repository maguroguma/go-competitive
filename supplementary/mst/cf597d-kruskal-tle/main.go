...
func main() {
	...
	for i := int64(0); i < n; i++ {
		for j := i + 1; j < n; j++ {
			...
			ecost := coef * dist
			es = append(es, Edge{key: ecost, from: ci.id, to: cj.id, cost: ecost})
		}
	}
	// id:0のダミーノードにダミーエッジをはる
	for i := int64(0); i < n; i++ {
		es = append(es, Edge{key: C[i], from: 0, to: i + 1, cost: C[i]})
	}
	v = 2000 + 5
...
func kruskal() int64 {
	...
	for _, e := range L {
		...
		if !uf.Same(int(e.from), int(e.to)) {
			uf.Unite(int(e.from), int(e.to))
			res += e.cost

			// 片方がダミーノードかどうかで保存先を分ける
			if e.from == 0 || e.to == 0 {
				ansCityIds = append(ansCityIds, Max(e.from, e.to))
			} else {
				connectedCities = append(connectedCities, []int64{e.from, e.to})
			}
		}
	}
