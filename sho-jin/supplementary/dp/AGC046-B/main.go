...
func main() {
	...
	for i := a; i <= c; i++ {
		for j := b; j <= d; j++ {
			// 0->0
			// 「一番上の行に黒が複数ある」->「複数ある」は、i行のうちいずれの行を選択してもいい
			// dp[a][b][0]のとき素直に考えるとおかしく見えるが、
			// a*bのマス目はすべて真っ黒と考えると納得できる（そもそもa*bの状態はかなり特殊と見る必要がある）
			dp[i][j+1][0] += dp[i][j][0] * i
			dp[i][j+1][0] %= MOD

			// 1->0
			// 「一番上の行に黒が1個ある」->「複数ある」なので、その一番上の行を選ぶしかない
			dp[i][j+1][0] += dp[i][j][1]
			dp[i][j+1][0] %= MOD
