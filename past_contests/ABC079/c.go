...
func main() {
	tmp := NextRunesLine()
	a, _ = strconv.Atoi(string(tmp[0]))
	b, _ = strconv.Atoi(string(tmp[1]))
	c, _ = strconv.Atoi(string(tmp[2]))
	d, _ = strconv.Atoi(string(tmp[3]))
	answers := []int{}
	answers = append(answers, a+b+c+d)
	answers = append(answers, a+b+c-d)
	answers = append(answers, a+b-c+d)
	answers = append(answers, a+b-c-d)
	answers = append(answers, a-b+c+d)
	...
	for idx, ans := range answers {
		if ans == 7 {
			...
			fmt.Println(strconv.Itoa(a) + string(ops[0]) + strconv.Itoa(b) + string(ops[1]) + strconv.Itoa(c) + string(ops[2]) + strconv.Itoa(d) + "=7")
