...
var dp [10000 + 5]string

func main() {
	barNums := []int{-1, 2, 5, 5, 4, 5, 6, 3, 7, 6}
	n, m = ReadInt(), ReadInt()
	A = ReadIntSlice(m)
	...
	for i := 1; i <= n; i++ {
		for _, a := range A {
			matchNum := barNums[a]
			if i-matchNum >= 0 && dp[i-matchNum] != "null" {
				currentStr := dp[i-matchNum] + strconv.Itoa(a)
				// fmt.Println(currentStr)
				// fmt.Printf("len(dp[%d]): %d, currentStr: %s\n", i, dp[i], currentStr)
				if dp[i] == "null" || len(dp[i]) < len(currentStr) || (len(dp[i]) == len(currentStr) && dp[i] < currentStr) {
					dp[i] = currentStr
				}
			}
		}
