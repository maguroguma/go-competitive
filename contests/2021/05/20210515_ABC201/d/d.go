...
func main() {
	...
	for i := h - 2; i >= 0; i-- {
		for j := w - 2; j >= 0; j-- {
			...
			co := B[i][j]
			if C[i][j] == 'T' {
				dp[i][j] = max(dp[ni][j]+co, dp[i][nj]+co)
			} else {
				dp[i][j] = min(dp[ni][j]-co, dp[i][nj]-co)
			}
		}
	}
...
// 				if isTaka {
// 					chmax(&dp[ni][j], dp[i][j]+co)
// 				} else {
// 					chmin(&dp[ni][j], dp[i][j]+co)
// 				}
// 			}
// 			if nj < w {
// 				if A[i][nj] == '+' {
...
// 				if isTaka {
// 					chmax(&dp[i][nj], dp[i][j]+co)
// 				} else {
// 					chmin(&dp[i][nj], dp[i][j]+co)
// 				}
// 			}
// 		}
// 	}
