...
// dp[S][v]:
// すでに訪れた頂点集合（スタート地点0はまだ訪れていないものとする）がS、
// 現在vにいる状態から残りのすべての頂点を巡って頂点0に帰るようなパスの、
// 重みの最小値
var dp [1 << 15][15]int // メモ化テーブル
...
func main() {
	...
	// 漸化式に従って順に計算
	for S := (1 << uint(n)) - 2; S >= 0; S-- {
		for v := 0; v < n; v++ {
			for u := 0; u < n; u++ {
				if !((S>>uint(u))&1 == 1) {
					dp[S][v] = Min(dp[S][v], dp[S|1<<uint(u)][u]+d[v][u])
				}
			}
		}
	}
