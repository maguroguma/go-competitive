...
func main() {
	...
	P = []int{}
	for i := 0; i < n; i++ {
		S := string(ReadRuneSlice())
		p := ReadInt()

		// if _, ok := memo[S]; ok {
		// 	memo[S] = append(memo[S], p)
		// } else {
		// 	memo[S] = []int{}
		// 	memo[S] = append(memo[S], p)
		// }
	...
	for _, str := range SS {
		fmt.Println(str)
		// arr := memo[str]
		fmt.Println(memo[str])
		sort.Sort(sort.Reverse(sort.IntSlice(memo[str])))
		for _, v := range memo[str] {
			fmt.Println("---")
			fmt.Println(v)
			ans = append(ans, str+strconv.Itoa(v))
		}
	}
	fmt.Println(ans)
}
