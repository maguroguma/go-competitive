...
func main() {
	...
	patterns := CombinationPatterns(ids, 9)
	ans := 0
...
func sub(ids []int) int {
	...
	for i := 0; i < m; i++ {
		...
		resBits := mask & bits
		// PrintDebug("mask:%s, bits:%s\n", string(ZeroPaddingRuneSlice(mask, n)), string(ZeroPaddingRuneSlice(bits, n)))
		// if PopCount(bits) == PopCount(resBits) {
		if PopCount(resBits) >= 3 {
			res += B[i]
		}
	}
