...
func main() {
	...
	for i := 0; i < len(memo); i++ {
		for j := 0; j < 2; j++ {
			ith := (len(memo) - 1) - i
			num := memo[ith]
			diff0 := PowInt(2, ith) * num
			diff1 := PowInt(2, ith) * (n - num)

			// fmt.Println(ith)

			kIthBit := GetNthBit(k, ith)
			if j == 1 {
				// 上位の確定桁から確実に小さいことがわかっているため、i桁目も自由に選べる
				if dp[i][j] != -1 {
					ChMax(&dp[i+1][j], dp[i][j]+Max(diff0, diff1))
				}
			} else {
				// j==0
				if dp[i][j] != -1 {
					// kのi桁目のビットにかかわらずlessフラグ0->0は考慮する
					// ChMax(&dp[i+1][j], dp[i][j] + diff0)
					if kIthBit == 1 {
						ChMax(&dp[i+1][1], dp[i][j]+diff0) // 0を選ぶことでlessフラグを立てる遷移
						ChMax(&dp[i+1][j], dp[i][j]+diff1) // 1に揃えてlessフラグを降ろしたままの遷移
					} else {
						ChMax(&dp[i+1][j], dp[i][j]+diff0) // kのビットが立っていない場合はlessフラグを降ろしたまま0を選ぶしかない
					}
				}
			}
		}
	...
	// fmt.Println(dp)
	fmt.Println(Max(dp[len(memo)][0], dp[len(memo)][1]))
}
