...
func main() {
	...
	// }
	// if num == 1 || num == 0 {
	// 	ans := gmin(c1.r*c1.r*G_PI, c2.r*c2.r*G_PI)
	// 	printf("%.10f\n", ans)
	// 	return
	// }
...
// 扇形の弦と弧の間の面積
// https://kmjp.hatenablog.jp/entry/2015/11/29/0930
// ???
func sub(c *Circle, a, b *Point) float64 {
	// angle := Angle(b, c.p, a)
	// all := (G_PI * c.r * c.r) * (angle / (2.0 * G_PI))
	// tri := gabs(Cross(a.Minus(c.p), b.Minus(c.p))) * 0.5
	// return all - tri

	res := 0.0
	angle := Angle(b, c.p, a)
	res += angle * c.r * c.r
	res -= c.r * gsin(angle) * c.r * gcos(angle)
	return res
}
...
// 	// return gmin(theta, 2.0*gacos(-1.0)-theta)
// }
// a-b-cの角度のうち小さい方を返す
func Angle(a, b, c *Point) float64 {
