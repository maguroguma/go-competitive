...
func (l byKey) Less(i, j int) bool {
	return l.RoomList[i].key < l.RoomList[j].key
}
...
func (l RoomList) Less(i, j int) bool {
	if l[i].val < l[j].val {
		...
		return l[i].idx < l[j].idx
	}
}

// how to use
// L := make(RoomList, 0, 200000+5)
// L = append(L, &Room{key: intValue})
...
func main() {
	...
	L := RoomList{}
	for i := 1; i < n; i++ {
		L = append(L, &Room{key: A[i], val: A[i], idx: i})
	}
	sort.Stable(L)

	for i := 0; i < len(L); i++ {
		PrintfDebug("val: %d, idx: %d\n", L[i].val, L[i].idx)
	}
