...
func main() {
	...
	sr, sc = ReadInt(), ReadInt()
	S, T = ReadRuneSlice(), ReadRuneSlice()

	ld, rd, ud, dd := AbsInt(0-sc), AbsInt((w+1)-sc), AbsInt(0-sr), AbsInt((h+1)-sr)

	TL, TR, TU, TD := make([]int, n), make([]int, n), make([]int, n), make([]int, n)
	AL, AR, AU, AD := make([]int, n), make([]int, n), make([]int, n), make([]int, n)
	if S[n-1] == 'L' {
		TL[n-1] = 1
	} else if S[n-1] == 'R' {
		TR[n-1] = 1
	...
	// fmt.Println(TL, TR, TU, TD)
	// fmt.Println(AL, AR, AU, AD)

	// sr は縦、sc は横
	for i := 0; i < n; i++ {
		tr, ar := S[i], T[i]
		if tr == 'L' && (AbsInt((ld-1)-(rd+1)) > AbsInt(ld-rd) || (AbsInt((ld-1)-(rd+1)) == AbsInt(ld-rd) && AR[i] < AL[i])) {
			ld--
			rd++
			sc--
		}
		if tr == 'R' && (AbsInt((rd-1)-(ld+1)) > AbsInt(ld-rd) || (AbsInt((rd-1)-(ld+1)) == AbsInt(ld-rd) && AR[i] > AL[i])) {
			rd--
			ld++
			sc++
		}
		if tr == 'U' && (AbsInt((ud-1)-(dd+1)) > AbsInt(ud-dd) || (AbsInt((ud-1)-(dd+1)) == AbsInt(ud-dd) && AU[i] > AD[i])) {
			ud--
			dd++
			sr--
		}
		if tr == 'D' && (AbsInt((dd-1)-(ud+1)) > AbsInt(ud-dd) || (AbsInt((dd-1)-(ud+1)) == AbsInt(ud-dd) && AU[i] < AD[i])) {
			dd--
			ud++
			sr++
		}
		...
		if ar == 'L' && (AbsInt((ld-1)-(rd+1)) < AbsInt(ld-rd) || (AbsInt((ld-1)-(rd+1)) == AbsInt(ld-rd) && i <= n-2 && TR[i+1] > TL[i+1])) {
			ld--
			rd++
			sc--
		}
		if ar == 'R' && (AbsInt((rd-1)-(ld+1)) < AbsInt(ld-rd) || (AbsInt((rd-1)-(ld+1)) == AbsInt(ld-rd) && i <= n-2 && TR[i+1] < TL[i+1])) {
			rd--
			ld++
			sc++
		}
		if ar == 'U' && (AbsInt((ud-1)-(dd+1)) < AbsInt(ud-dd) || (AbsInt((ud-1)-(dd+1)) == AbsInt(ud-dd) && i <= n-2 && TU[i+1] < TD[i+1])) {
			ud--
			dd++
			sr--
		}
		if ar == 'D' && (AbsInt((dd-1)-(ud+1)) < AbsInt(ud-dd) || (AbsInt((dd-1)-(ud+1)) == AbsInt(ud-dd) && i <= n-2 && TU[i+1] > TD[i+1])) {
			dd--
			ud++
			sr++
		}
