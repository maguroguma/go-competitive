...
func main() {
	...
	memo := make(ItemList, n)
	for i := 0; i < n; i++ {
		a, b := A[i], B[i]
		item := &Item{key: a * b, idx: i}
		memo[i] = item
	}

	sort.Sort(sort.Reverse(memo))

	candIdx := memo[0].idx
	maxa, maxb := A[candIdx], B[candIdx]
	for i := 0; i < n; i++ {
		if i == candIdx {
			continue
		}
		...
		a, b := A[i], B[i]
		if CeilInt(maxa, b) > CeilInt(a, maxb) {
			continue
		} else {
			fmt.Println(-1)
			return
