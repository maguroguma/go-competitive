...
func main() {
	...
	for _, a := range A {
		a--
		astrs = append(astrs, string(S[a]))
	}
	sort.Sort(sort.StringSlice(astrs))
	// fmt.Println(astrs)

	maxIdx := 0
	for i := 0; ; i++ {
		if i == len(astrs[0]) || i == len(astrs[len(astrs)-1]) {
			break
		}

		if astrs[0][i] == astrs[len(astrs)-1][i] {
			maxIdx = i
		} else {
			break
		}
	...
	maxCandidate := S[A[0]-1][:maxIdx+1]
	// fmt.Println(string(maxCandidate))

	ng, ok := -1, len(maxCandidate)
	for int(math.Abs(float64(ok-ng))) > 1 {
		mid := (ok + ng) / 2

		// カウントしてk+1個以上マッチしたらアウト
		isOK := sub(maxCandidate[:mid])
	...
	if sub(maxCandidate[:ok]) {
		fmt.Println(string(maxCandidate[:ok]))
	} else {
		fmt.Println(-1)
	}
}

// 与えられた文字列が欲しいものだけにマッチするかどうか
func sub(R []rune) bool {
	partial := 0
	all := 0
	...
	for _, a := range A {
		...
		if string(S[a][:l]) == string(R) {
			partial++
		}
	}

	return ((all == partial) && (partial == k))
}
