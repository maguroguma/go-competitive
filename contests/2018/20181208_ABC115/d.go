...
func main() {
	...
	fmt.Println(recursion(n, x))
}

func recursion(n, x int) int {
	if n == 0 {
		if x == 1 {
			return 1
		} else {
	...
	if x == 1 {
		return 0
	} else if 1 < x && x <= 1+totalLen[n-1] {
		return recursion(n-1, x-1)
	} else if x == 2+totalLen[n-1] {
		return 1 + pLen[n-1]
	} else if 2+totalLen[n-1] < x && x <= 2+2*totalLen[n-1] {
		return pLen[n-1] + 1 + recursion(n-1, x-(2+totalLen[n-1]))
	} else if x == 3+2*totalLen[n-1] {
		return 1 + 2*pLen[n-1]
	}

	return -1 // Goの言語仕様のため必要
}
