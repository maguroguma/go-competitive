...
func main() {
	...
	for i := 0; i < n; i++ {
		var minus, plus int
		if i == 0 {
			minus = (AbsInt(0 - A[i])) + AbsInt(A[i]-A[i+1])
			plus = AbsInt(0 - A[i+1])
		} else if i == n-1 {
			minus = AbsInt(A[i-1]-A[i]) + AbsInt(A[i]-0)
			plus = AbsInt(A[i-1] - 0)
		} else {
			minus = AbsInt(A[i-1]-A[i]) + AbsInt(A[i]-A[i+1])
			plus = AbsInt(A[i-1] - A[i+1])
		}
		fmt.Println(sum - minus + plus)
	}
}
