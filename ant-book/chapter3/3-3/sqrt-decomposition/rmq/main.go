...
// [x]: https://onlinejudge.u-aizu.ac.jp/problems/DSL_2_A

const sqrtN = 512

type SqrtDecompositionRMQ struct {
	n, k            int
	data, bucketMin []int
}

func NewSqrtDecompositionRMQ(A []int) *SqrtDecompositionRMQ {
	sd := new(SqrtDecompositionRMQ)
	sd.n = len(A)
	...
	sd.bucketMin = make([]int, sd.k)
	for i := 0; i < sd.k; i++ {
		minVal := 1 << 60
		for j := i * sqrtN; j < (i+1)*sqrtN; j++ {
			minVal = int(math.Min(float64(minVal), float64(sd.data[j])))
		}
		sd.bucketMin[i] = minVal
	}

	return sd
}

func (sd *SqrtDecompositionRMQ) Update(x, y int) {
	k := x / sqrtN
	sd.data[x] = y
	minVal := y
...
// [x, y)
func (sd *SqrtDecompositionRMQ) Find(x, y int) int {
	minVal := 1 << 60
	...
	for k := 0; k < sd.k; k++ {
		...
		if x <= l && r <= y {
			...
			a := int(math.Max(float64(l), float64(x)))
			b := int(math.Min(float64(r), float64(y)))
			for i := a; i < b; i++ {
				minVal = int(math.Min(float64(minVal), float64(sd.data[i])))
			}
		}
