...
var dp [2][1 << 15]int // DPテーブル（再利用する）

func solve() {
	current, next := &dp[0], &dp[1]
	current[0] = 1
	for i := n - 1; i >= 0; i-- {
		for j := m - 1; j >= 0; j-- {
			for used := 0; used < 1<<uint(m); used++ {
				if (used>>uint(j))&1 == 1 || color[i][j] {
					// (i, j)にはブロックを置く必要がない
					next[used] = current[used & ^(1<<uint(j))]
				} else {
					// 2通りの向きを試す
					res := 0
					// 横向き
					if j+1 < m && !(used>>uint(j+1)&1 == 1) && !color[i][j+1] {
						res += current[used|1<<uint(j+1)]
					}
					// 縦向き
					if i+1 < n && !color[i+1][j] {
						res += current[used|1<<uint(j)]
					}
					next[used] = res % M
				}
			}
