...
func main() {
	n, k, l = ReadInt(), ReadInt(), ReadInt()
	roadParents, railParents = make([]int, n+1), make([]int, n+1)
	initialize(roadParents)
	initialize(railParents)
	for i := 0; i < k; i++ {
		p, q := ReadInt(), ReadInt()
		unite(p, q, roadParents)
	}
	for i := 0; i < l; i++ {
		r, s := ReadInt(), ReadInt()
		unite(r, s, railParents)
	}

	memo := make(map[pair]int)
	for i := 1; i <= n; i++ {
		p := pair{a: root(roadParents[i], roadParents), b: root(railParents[i], railParents)}
		memo[p]++
	}
	for i := 1; i <= n; i++ {
		p := pair{a: root(roadParents[i], roadParents), b: root(railParents[i], railParents)}
