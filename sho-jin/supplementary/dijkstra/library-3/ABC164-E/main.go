...
func toVid(bid, coin int) int {
	return coin*n + bid
}
func toBid(vid int) int {
	return vid % n
}
func toCoin(vid int) int {
	return vid / n
}
...
func main() {
	...
	vinf := Value{INF_B60}
	less := func(l, r Value) bool { return l.time < r.time }
	transit := func(cv *Vertex, AG [][]Edge) []*Vertex {
		res := []*Vertex{}

		cbid := toBid(cv.vid)
		cc := toCoin(cv.vid)
		// コインが足りている場合は、消費して移動する
		for _, e := range AG[cbid] {
			nbid := e.to
			if cc-e.w.coin >= 0 {
				nc := cc - e.w.coin
				nvid := toVid(nbid, nc)
				nval := Value{cv.v.time + e.w.time}
				res = append(res, &Vertex{nvid, nval})
			}
		}
		// 両替を一度だけする
		c, d := C[cbid], D[cbid]
		nc := min(CMAX, cc+c)
		nvid := toVid(cbid, nc)
		nval := Value{cv.v.time + d}
		res = append(res, &Vertex{nvid, nval})

		// 必要な分だけ両替の遷移を考慮する -> TLE!!!
		// c, d := C[cbid], D[cbid]
		// for i := 1; i <= CMAX; i++ {
		// 	nval := Value{cv.v.time + i*d}
		// 	if i*c+cc >= CMAX {
		// 		// nc := CMAX
		// 		// nvid := toVid(cbid, nc)
		// 		// res = append(res, &Vertex{nvid, nval})
	...
	SS := []StartPoint{{vid: toVid(0, min(s, CMAX)), vzero: Value{0}}}
	N := n*CMAX + 100
