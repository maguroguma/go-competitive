...
// http://prognote.web.fc2.com/win_c_cpp/tri_outcircle/index.html
...
func sub(a, b *Point) *Line {
	m := a.Add(b).Mul(0.5)
	// n := m.Add(RotateTheta(G_PI/2.0, b.Minus(a)))
	n := m.Add(rot90(b.Minus(a)))
	return NewLine(m, n)
}

func rot90(p *Point) *Point {
...
func TangentCirclePoint(c1 *Circle, p2 *Point) [2]*Point {
	c2 := NewCircle(p2, gsqrt(c1.p.Minus(p2).Norm()-c1.r*c1.r))
