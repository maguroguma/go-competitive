...
func main() {
	...
	for i := 0; i < n; i++ {
		tmp := ReadRuneSlice()
		v, _ := strconv.Atoi(string(tmp[1]))
		c, d := NewCard(v, tmp[0]), NewCard(v, tmp[0])
		C, D = append(C, c), append(D, d)
		memo[string(tmp)] = i
	}
...
// 安定ソートであるバブルソートの結果と比較するのがベスト
func IsStable(C []*Card, n int) bool {
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			if C[i].value == C[j].value && memo[C[i].ToString()] > memo[C[j].ToString()] {
				return false
			}
		}
	}
...
func NewCard(value int, suit rune) *Card {
	c := new(Card)
	c.suit = suit
	c.value = value
	return c
...
func (c *Card) ToString() string {
	res := ""
	res += string(c.suit)
	res += strconv.Itoa(c.value)
	return res
}

func bubbleSort(C []*Card, n int) {
	for i := 0; i < n; i++ {
		for j := n - 1; j > i; j-- {
...
func selectionSort(C []*Card, n int) {
	for i := 0; i < n; i++ {
		minj := i
		for j := i; j < n; j++ {
...
func PrintCards(C []*Card, n int) {
	for i := 0; i < len(C); i++ {
		if i == len(C)-1 {
			fmt.Printf("%s\n", C[i].ToString())
		} else {
			fmt.Printf("%s ", C[i].ToString())
		}
	}
}
