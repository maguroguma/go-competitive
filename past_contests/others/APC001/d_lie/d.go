...
func main() {
	...
	for i := 0; i < n; i++ {
		rid := uf.Root(i)
		node := &Node{key: rid, rid: rid, id: i}
		L = append(L, node)
	}
	sort.Stable(byKey{L})
	for i := 0; i < len(L); i++ {
		PrintDebug("rid: %d, id: %d\n", L[i].rid, L[i].id)
	}

	roots := []int{}
	costs := []int{}
	for i := 0; i < len(L); i++ {
		roots = append(roots, L[i].rid)
		costs = append(costs, A[L[i].id])
	}
	PrintDebug("%v\n", roots)
	PrintDebug("%v\n", costs)

	_, cnts := RunLengthEncoding(roots)
	csums := make([]int, len(cnts)+1)
	for i := 0; i < len(cnts); i++ {
		csums[i+1] = csums[i] + cnts[i]
	}
	PrintDebug("%v\n", csums)
	...
	// ti := T{v: 1<<31 - 1, idx: -1}
	ti := T{v: INF_BIT60, idx: -1}
	st := NewSegmentTree(n, f, ti)
	for i := 0; i < n; i++ {
		st.Set(i, T{v: costs[i], idx: i})
	}
	st.Build()

	ans := 0
	for i := 1; i < len(csums)-1; i++ {
		l := csums[i]
		r := csums[i+1]
		lmin := st.Query(0, l)
		rmin := st.Query(l, r)
		if lmin.v == INF_BIT60 || rmin.v == INF_BIT60 {
			fmt.Println("Impossible")
			return
		}
		ans += lmin.v + rmin.v
		st.Update(lmin.idx, T{v: INF_BIT60, idx: lmin.idx})
		st.Update(rmin.idx, T{v: INF_BIT60, idx: rmin.idx})
	}
	fmt.Println(ans)
}
