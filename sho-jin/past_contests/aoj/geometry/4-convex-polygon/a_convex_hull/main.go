...
func main() {
	...
	C := ConvexHullAOJ(P)
	// sort.Slice(cvh, func(i, j int) bool {
	// 	if cvh[i].y < cvh[j].y {
	// 		return true
	// 	} else if cvh[i].y > cvh[j].y {
	// 		return false
	// 	} else {
	// 		return cvh[i].x < cvh[j].x
	// 	}
	// })
	sid := 0
	for i := 1; i < len(C); i++ {
		if C[i].y < C[sid].y {
			sid = i
		} else if C[i].y > C[sid].y {
		} else if C[i].x < C[sid].x {
			sid = i
		}
	}
	...
	// }
	for i := 0; i < len(C); i++ {
		printf("%v %v\n", C[(sid+i)%len(C)].x, C[(sid+i)%len(C)].y)
	}
}
...
// 多角形包含判定
// https://ei1333.github.io/algorithm/contains.html
func Contains(P []Coord, q Coord) int {
	const (
		OUT, ON, IN = 0, 1, 2
	)
	...
	for i := 0; i < n; i++ {
		...
		cross := a.det(b)
		if a.y <= 0 && 0 < b.y && !_equal(cross, 0.0) && cross < 0.0 {
			isIn = !isIn
		}
		if _equal(cross, 0.0) && a.dot(b) <= 0.0 {
			return ON
		}
	}
...
func ConvexHullAOJ(ps []Coord) []Coord {
	sort.Slice(ps, func(i, j int) bool {
		if ps[i].x != ps[j].x {
			return ps[i].x < ps[j].x
		}
	...
	for i := 0; i < n; i++ {
		for k > 1 {
			...
			if _equal(lp.det(rp), 0.0) || lp.det(rp) > 0.0 {
				break
			}
			k--
		}
...
func DiameterOfConvexHullByCalipers(ps []Coord) float64 {
