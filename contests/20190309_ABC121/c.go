...
func main() {
	...
	shopList := make(MonoList, n)
	for i := 0; i < n; i++ {
		shopList[i] = &Mono{key: i, value: A[i]}
	}
	sort.Sort(shopList)
	...
	residualDrink := m
	for i := 0; i < n; i++ {
		nedan := shopList[i].value
		shopId := shopList[i].key
		maxNum := B[shopId]

		if residualDrink >= maxNum {
			ans += maxNum * nedan
			residualDrink -= maxNum
		} else {
			ans += residualDrink * nedan
			break
		}
	}
