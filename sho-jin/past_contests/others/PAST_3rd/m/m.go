/*
URL:
https://atcoder.jp/contests/past202005/tasks/past202005_m
*/
...
var (
	...
	must    [100000 + 50]bool // 訪れなければいけないかどうか
	visited [100000 + 50]bool // 訪問済みかどうか
)
...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		for j := 0; j < k; j++ {
			...
			// 同じ階層は必ず双方向につなぐ
			ul, vl := u+n*j, v+n*j
			G[ul] = append(G[ul], Edge{to: vl, cost: 1})
			G[vl] = append(G[vl], Edge{to: ul, cost: 1})

			// ターゲットなら上方向へつなぐ
			if must[u] {
				G[vl] = append(G[vl], Edge{to: u + n*(j+1), cost: 1})
			}
			if must[v] {
				G[ul] = append(G[ul], Edge{to: v + n*(j+1), cost: 1})
			}
		}
	}

	total := n * (k + 1)
	dp, parents := dijkstra(s, total, G[:total])
	ans := INF_DIJK
	for i := 0; i < n; i++ {
		PrintfDebug("id: %d, cost: %d\n", (i+n*k)%n, dp[i+n*k])
		...
		for p >= 0 {
			rev = append(rev, p%n)
			revOrg = append(revOrg, p)
			p = parents[p]
		}
		PrintfDebug("Reverse: %v\n", Reverse(rev))
		PrintfDebug("Reverse Original: %v\n", Reverse(revOrg))
		// }
	}
	fmt.Println(ans)
}
...
func dijkstra(sid, n int, AG [][]Edge) (dp, parents []int) {
	...
	for pq.Len() > 0 {
		...
		for _, e := range AG[pop.id] {
			...
			// 行き先がターゲット、かつ、まだ未訪問
			// orgCityID := e.to % n
			// if must[orgCityID] && !visited[orgCityID] {
