...
func main() {
	...
	temp := make(GasolinStandPQ, 0, 100000+1)
	pq := &temp
	heap.Init(pq)
	...
	for i := 0; i < n; i++ {
		...
		// 十分な量になるまでガソリンを補給（ガソリン量が大きいものから優先的に使うのが最大のポイント！）
		for tank-d < 0 {
			if len(*pq) == 0 {
				fmt.Println(-1)
				return
		...
		tank -= d
		pos = A[i]
		heap.Push(pq, &GasolinStand{priority: B[i], gas: B[i], dist: A[i]})
	}

	fmt.Println(ans)
}

type GasolinStand struct {
	priority  int
	gas, dist int
}
type GasolinStandPQ []*GasolinStand

func (pq GasolinStandPQ) Len() int           { return len(pq) }
func (pq GasolinStandPQ) Less(i, j int) bool { return pq[i].priority > pq[j].priority } // <: ASC, >: DESC
func (pq GasolinStandPQ) Swap(i, j int) {
	pq[i], pq[j] = pq[j], pq[i]
}
func (pq *GasolinStandPQ) Push(x interface{}) {
	item := x.(*GasolinStand)
	*pq = append(*pq, item)
}
func (pq *GasolinStandPQ) Pop() interface{} {
	old := *pq
	n := len(old)
	item := old[n-1]
	*pq = old[0 : n-1]
...
// pq := &temp
// heap.Init(pq)
// heap.Push(pq, &GasolinStand{priority: intValue})
// popped := heap.Pop(pq).(*GasolinStand)
