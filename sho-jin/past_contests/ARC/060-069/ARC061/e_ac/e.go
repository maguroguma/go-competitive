...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		for _, e := range edges[i] {
			dp[e.tid][e.comp] = INF_BIT60
			flags[e.tid][e.comp] = false
		}
	}
	...
	dp[0][-1] = 0
	// flags[0][-1] = true
	heap.Push(pq, &Station{priority: 0, befComp: -1, id: 0})
	for pq.Len() > 0 {
		cs := heap.Pop(pq).(*Station)
		flags[cs.id][cs.befComp] = true

		for _, e := range edges[cs.id] {
			if cs.befComp == -1 {
				if !flags[e.tid][e.comp] && dp[e.tid][e.comp] > cs.priority+1 {
					dp[e.tid][e.comp] = cs.priority + 1
					heap.Push(pq, &Station{priority: dp[e.tid][e.comp], befComp: e.comp, id: e.tid})
				}
			} else if cs.befComp != e.comp {
				if !flags[cs.id][-1] && dp[cs.id][-1] > cs.priority {
					dp[cs.id][-1] = cs.priority
					heap.Push(pq, &Station{priority: dp[cs.id][-1], befComp: -1, id: cs.id})
				}
			} else {
				if !flags[e.tid][e.comp] && dp[e.tid][e.comp] > cs.priority {
					dp[e.tid][e.comp] = cs.priority
					heap.Push(pq, &Station{priority: dp[e.tid][e.comp], befComp: e.comp, id: e.tid})
				}
			}
		}
