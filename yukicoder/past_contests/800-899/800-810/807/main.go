...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		edges[a] = append(edges[a], Edge{t: b, d: c})
		edges[b] = append(edges[b], Edge{t: a, d: c})
	}
	...
	pq := &temp
	heap.Init(pq)
	heap.Push(pq, &Node{priority: dp[0][0], id: 0, state: 0})
	...
	for pq.Len() > 0 {
		...
		for _, next := range edges[node.id] {
			if node.state == 0 {
				if dp[next.t][0] > node.priority+next.d {
					dp[next.t][0] = node.priority + next.d
					heap.Push(pq, &Node{priority: dp[next.t][0], id: next.t, state: 0})
				}
				if dp[next.t][1] > node.priority {
					dp[next.t][1] = node.priority
					heap.Push(pq, &Node{priority: dp[next.t][1], id: next.t, state: 1})
				}
			} else {
				if dp[next.t][1] > node.priority+next.d {
					dp[next.t][1] = node.priority + next.d
					heap.Push(pq, &Node{priority: dp[next.t][1], id: next.t, state: 1})
				}
			}
		}
