...
func main() {
	...
	temp := make(FactoryPQ, 0, 100000+5)
	pq := &temp
	heap.Init(pq)
	for i := 0; i < n; i++ {
		obj := &Factory{priority: A[i], idx: i}
		heap.Push(pq, obj)
	}

	ans := 0
	for i := 0; i < k; i++ {
		popped := heap.Pop(pq).(*Factory)
		ans += popped.priority

		popped.priority += B[popped.idx]
		heap.Push(pq, popped)
	}
...
// how to use
// temp := make(FactoryPQ, 0, 100000+1)
// pq := &temp
// heap.Init(pq)
// heap.Push(pq, &Factory{priority: intValue})
// popped := heap.Pop(pq).(*Factory)
