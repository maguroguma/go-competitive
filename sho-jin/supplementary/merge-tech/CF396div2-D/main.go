...
func main() {
	...
	uf := NewDisjointSetUnion(n)
	G = make([][]Edge, n)
	cum = make([]int, n)

	for i := 0; i < m; i++ {
		com, s, t := readi(), reads(), reads()
		a, b := dict[s], dict[t]
		...
		// マージするときはDFSで小さい方のみ更新する
		if !uf.Same(a, b) {
			printf("YES\n")

			// bを小さくする
			as, bs := uf.Size(a), uf.Size(b)
			if as < bs {
				a, b = b, a
			}
			uf.Merge(a, b)
			G[a] = append(G[a], Edge{to: b, cost: ec})
			G[b] = append(G[b], Edge{to: a, cost: ec})
			dfs(b, a, cum[a]^ec)
...
func dfs(cid, pid int, cur int) {
	cum[cid] = cur
	for _, e := range G[cid] {
		if e.to == pid {
			continue
		}
		dfs(e.to, cid, cur^e.cost)
	}
}
