...
func solveLarge() {
	...
	for i := 9; i <= n-1; i++ {
		A = append(A, [2]int{i, n})
	}

	val := n
	for val > 1 {
		A = append(A, [2]int{n, 8})
		val = Kiriage(val, 8)
	}

	for i := 3; i <= min(7, n); i++ {
		A = append(A, [2]int{i, 8})
	}

	A = append(A, [2]int{8, 2})
	A = append(A, [2]int{8, 2})
	A = append(A, [2]int{8, 2})
...
func solve() {
	...
	if maxi < n {
		val := n
		for val > 1 {
			A = append(A, [2]int{n, maxi})
			val = Kiriage(val, maxi)
		}
	}

	for i := len(B) - 1; i >= 2; i-- {
		A = append(A, [2]int{B[i], B[i-1]})
	}
	for i := len(B) - 1; i >= 2; i-- {
		A = append(A, [2]int{B[i], 2})
	}
