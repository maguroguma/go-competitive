...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		ans += trie.accum(S[i][:len(S[i])-1], memo)
...
func (t *Trie) accum(word string, memo [ALPHABET_NUM]int) int {
	res := 0
	...
	for _, r := range word {
		for i := 0; i < _TRIE_CHAR_SIZE; i++ {
			nid := t.nodes[nodeID].next[i]
			if nid != -1 && len(t.nodes[nid].accept) > 0 && memo[i] > 0 {
				res++
			}
		}
