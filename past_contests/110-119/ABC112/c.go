...
func main() {
	n = ReadInt()
	for i := 0; i < n; i++ {
		x, y, h := ReadInt(), ReadInt(), ReadInt()
		X, Y, H = append(X, x), append(Y, y), append(H, h)
	}

	heightList := make(MonoList, n)
	for i := 0; i < n; i++ {
		heightList[i] = &Mono{key: i, value: H[i]}
	}
	sort.Sort(sort.Reverse(heightList))

	for cx := 0; cx <= 100; cx++ {
		for cy := 0; cy <= 100; cy++ {

			// hを決定する
			hi := heightList[0].value
			xi, yi := X[heightList[0].key], Y[heightList[0].key]
			constant := -(AbsInt(xi-cx) + AbsInt(yi-cy))
			h := hi - constant
			// 検証
			for i := 0; i < n; i++ {
				xi, yi, hi = X[i], Y[i], H[i]
				constant = -AbsInt(xi-cx) - AbsInt(yi-cy)

				if hi != Max(h+constant, 0) {
					break
				}
