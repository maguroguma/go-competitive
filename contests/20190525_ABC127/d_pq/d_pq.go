...
func main() {
	...
	heap.Init(pq)
	for i := 0; i < n; i++ {
		heap.Push(pq, &Item{priority: A[i], value: A[i], num: 1})
	}
	for i := 0; i < m; i++ {
		heap.Push(pq, &Item{priority: C[i], value: C[i], num: B[i]})
	}

	answers := []int{}
	for len(answers) < n {
		popped := heap.Pop(pq).(*Item)
		for i := 0; i < popped.num; i++ {
			answers = append(answers, popped.value)
		}
	}

	fmt.Println(Sum(answers[:n]...))
}
