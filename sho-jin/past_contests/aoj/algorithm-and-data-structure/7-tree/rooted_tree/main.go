...
func main() {
	...
	for i := 0; i < n; i++ {
		T[i].p, T[i].l, T[i].r = NIL, NIL, NIL
	}

	for i := 0; i < n; i++ {
		id, k := ReadInt2()
		childlen := ReadIntSlice(k)
		l := NIL
		for j := 0; j < k; j++ {
			if j == 0 {
				T[id].l = childlen[j]
...
const MAX_NODES_NUM = 200000
const NIL = -1

var T [MAX_NODES_NUM + 5]Node
var D [MAX_NODES_NUM + 5]int

type Node struct {
	p, l, r int
}

func PrintNode(u int) {
