...
OUTER:
	...
	if ok {
		...
		for i := si; i >= 1; i-- {
			a := A[i-1]
			if dp[i-1][mod(cj-a, M)][ck] {
				B = append(B, i)
				cj = mod(cj-a, M)
			} else if dp[i-1][cj][mod(ck-a, M)] {
				C = append(C, i)
				ck = mod(ck-a, M)
			} else if dp[i-1][cj][ck] {
			} else {
				panic("WTF!")
			}
		}
		...
		printf("Yes\n")
		printf("%d %s\n", len(B), PrintIntsLine(B...))
		printf("%d %s\n", len(C), PrintIntsLine(C...))
