...
func main() {
	n, S = ReadInt(), ReadRuneSlice()
	cumsumOfW, cumsumOfE := [300001]int{}, [300001]int{}
	csw := 0
	cse := 0
	for i, s := range S {
		if s == 'W' {
	...
	for i := 0; i < n; i++ {
		tmpans := 0
		if i == 0 {
			tmpans = cumsumOfE[n-1] - cumsumOfE[0]
		} else if i == n-1 {
			tmpans = cumsumOfW[n-2]
		} else {
			tmpans = (cumsumOfW[i-1]) + (cumsumOfE[n-1] - cumsumOfE[i])
		}
		ans = Min(ans, tmpans)
	}
	fmt.Println(ans)
