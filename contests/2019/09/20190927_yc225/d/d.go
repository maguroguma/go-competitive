...
var beki [400000 + 10]int
var sumBeki [400000 + 10]int
...
func main() {
	...
	beki[0] = 1
	for i := 1; i < 400000+3; i++ {
		beki[i] = 2 * beki[i-1]
		beki[i] %= MOD
	}
	for i := 0; i < 400000+3; i++ {
		sumBeki[i+1] = sumBeki[i] + beki[i]
		sumBeki[i+1] %= MOD
	}

	for i := b + c - 1; i < a+b+c-1; i++ {
		m := sumBeki[i] * cf.C(i-1, c-1)
		m %= MOD
		m *= cf.C(i-c, b-1)
		m %= MOD
		ans += m
...
type CombFactorial struct {
	factorial, modFactorial [400001]int
}
