...
func ConvexHull(ps []P) []P {
	...
	k := 0               // 凸包の頂点数
	qs := make([]P, 2*n) // 構築中の凸包

	// 下側凸包の作成
	for i := 0; i < n; i++ {
		for k > 1 {
			lx := add(qs[k-1].x, -qs[k-2].x)
			ly := add(qs[k-1].y, -qs[k-2].y)
			lp := P{lx, ly}

			rx := add(ps[i].x, -qs[k-1].x)
			ry := add(ps[i].y, -qs[k-1].y)
			rp := P{rx, ry}
...
func solve_hard(ps []P) float64 {
	_cmp_x := func(p, q P) bool {
		if p.x != q.x {
			return p.x < q.x
		}
		return p.y < q.y
	...
	for i != sj || j != si {
		...
		lp, rp := qs[im], qs[i]
		lx, ly := add(lp.x, -rp.x), add(lp.y, -rp.y)
		left := P{lx, ly}

		lp, rp = qs[jm], qs[j]
		rx, ry := add(lp.x, -rp.x), add(lp.y, -rp.y)
		right := P{rx, ry}
