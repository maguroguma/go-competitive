...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		A[i], B[i], T[i], K[i] = a, b, t, k
	}

	G = make([][]Edge, n)
	for i := 0; i < m; i++ {
		a, b, t, k := A[i], B[i], T[i], K[i]
		G[a] = append(G[a], Edge{to: b, cost: t, interval: k})
		G[b] = append(G[b], Edge{to: a, cost: t, interval: k})
	}
...
func dijkstra(sid, n int, AG [][]Edge) (dp, parents []int) {
	...
	for pq.Len() > 0 {
		...
		for _, e := range AG[pop.id] {
			...
			curT := dp[pop.id]
			co := Kiriage(curT, e.interval)
			dipT := e.interval * co

			// if dp[e.to] > dp[pop.id]+e.cost {
			if dp[e.to] > dipT+e.cost {
