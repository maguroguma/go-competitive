...
func main() {
  ...
  for i := 0; i < m; i++ {
    x, y := ReadInt(), ReadInt()
    X[i], Y[i] = x-1, y-1 // 0-basedに変換
  }

  // 自分より早いうさぎの番号に応じたビットを立てたメモを作っておく
  memo := make([]int, n)
  for i := 0; i < m; i++ {
    x, y := X[i], Y[i]
    memo[y] |= 1 << uint(x)
  ...
  for i := 0; i < maxbits; i++ {
    // usabit = 注目するうさぎ（自分）
    for usaNth := 0; usaNth < n; usaNth++ {
      // (自分より早いうさビット列) & (今のビット列から自分自身を引いたもの) == (自分より早いうさビット列)
      if (i & (1<<uint(usaNth))) > 0 && memo[usaNth] & (i - 1<<uint(usaNth)) == memo[usaNth] {
        // fmt.Println(i - 1<<uint(usaNth))
        dp[i] += dp[i - 1<<uint(usaNth)]
      }
    }
  }
