...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		aa, bb := AbsInt(a), AbsInt(b)
		g := Gcd(aa, bb)

		// tmp := Iwashi{q: bb / aa, r: bb % aa, sign: sign}
		tmp := Iwashi{bunbo: aa / g, bunshi: bb / g, sign: sign}
	...
	for i := 0; i < n; i++ {
		...
		// key := Iwashi{q: aa / bb, r: aa % bb, sign: sign}
		key := Iwashi{bunbo: bb / g, bunshi: aa / g, sign: sign}
		cnt := memo[key]
		PrintfDebug("cnt: %v\n", cnt)

		res := modpow(2, n-(i+1)-cnt, MOD)
		res %= MOD
		answers[i] = res
	}
