...
func main() {
		...
		for j := 0; j < w; j++ {
			...
			for _, s := range steps {
				...
				if 0 <= ny && ny < h && 0 <= nx && nx < w && A[ny][nx] != '#' {
					...
					G[cid] = append(G[cid], Edge{nid, Weight{1}})
				}
			}
		}
	}
	...
	// 	for j := 0; j < len(R[v]); j++ {
	// 		from := R[v][j]
	// 		for k := j + 1; k < len(R[v]); k++ {
	// 			to := R[v][k]

	// 			G[from] = append(G[from], Edge{to, Weight{1}})
	// 			G[to] = append(G[to], Edge{from, Weight{1}})
	// 		}
	// 	}
	// }
	...
	estimate := func(cid int, cv Value, e Edge) Value {
		return Value{cv.v + e.w.v}
	}
	dijk := NewDijkstraSolver(vinf, less, estimate)

	SS := []StartPoint{{sid, Value{0}}}
	dp := dijk.Dijkstra(SS, N, G)

	if dp[gid].v >= INF_B60 {
		fmt.Println(-1)
	} else {
		fmt.Println(dp[gid].v)
	}
}
