...
func bruteForceAStar() {
	...
	for _, machine := range Machines {
		revRoute := AStar(machine.y, machine.x)
		...
		for i := len(revRoute) - 2; i >= 0; i-- {
			...
			if !IsSet[cy][cx] {
				...
				// 壁ないバージョン
				// if cy-ny == 1 || (cy == 0 && ny == n-1) {
				// 	outputs = append(outputs, direction{y: cy, x: cx, dir: 'U'})
				// } else if cy-ny == -1 || (cy == n-1 && ny == 0) {
				// 	outputs = append(outputs, direction{y: cy, x: cx, dir: 'D'})
				// } else if cx-nx == 1 || (cx == 0 && nx == n-1) {
				// 	outputs = append(outputs, direction{y: cy, x: cx, dir: 'L'})
				// } else if cx-nx == -1 || (cx == n-1 && nx == 0) {
				// 	outputs = append(outputs, direction{y: cy, x: cx, dir: 'R'})
				// }

				// また、直前と方向が同じであれば新しく矢印は設置しない
	...
	// }
	newOutputs := restructure(outputs)
	fmt.Printf("%d\n", len(newOutputs))
	for i := 0; i < len(newOutputs); i++ {
		fmt.Printf("%d %d %c\n", newOutputs[i].y, newOutputs[i].x, newOutputs[i].dir)
	}
}

var finalDirs [40][40]rune

func restructure(dirs []direction) []direction {
	for yidx := 0; yidx < n; yidx++ {
		for xidx := 0; xidx < n; xidx++ {
			if Blocks[yidx][xidx] {
				finalDirs[yidx][xidx] = 'b'
			} else {
				finalDirs[yidx][xidx] = 'x'
			}
		}
	}
	...
	for i := 0; i < len(dirs); i++ {
		yidx, xidx, dirRune := dirs[i].y, dirs[i].x, dirs[i].dir
		finalDirs[yidx][xidx] = dirRune
	}

	// L
	befDir := 'x'
	for yidx := 0; yidx < n; yidx++ {
		for xidx := n - 1; xidx >= 0; xidx-- {
			if finalDirs[yidx][xidx] != 'L' && finalDirs[yidx][xidx] != 'x' {
				befDir = finalDirs[yidx][xidx]
				continue
			}

			if befDir != 'L' && finalDirs[yidx][xidx] == 'L' {
				befDir = 'L'
				continue
			}

			if finalDirs[yidx][xidx] == 'L' && befDir == 'L' {
				finalDirs[yidx][xidx] = 'x'
			}
		}
	}
	...
	for yidx := 0; yidx < n; yidx++ {
		for xidx := 0; xidx < n; xidx++ {
			if finalDirs[yidx][xidx] != 'R' && finalDirs[yidx][xidx] != 'x' {
				befDir = finalDirs[yidx][xidx]
				continue
			}

			if befDir != 'R' && finalDirs[yidx][xidx] == 'R' {
				befDir = 'R'
				continue
			}

			if finalDirs[yidx][xidx] == 'R' && befDir == 'R' {
				finalDirs[yidx][xidx] = 'x'
			}
		}
	}
	...
	befDir = 'x'
	for xidx := 0; xidx < n; xidx++ {
		for yidx := n - 1; yidx >= 0; yidx-- {
			if finalDirs[yidx][xidx] != 'U' && finalDirs[yidx][xidx] != 'x' {
				befDir = finalDirs[yidx][xidx]
				continue
			}

			if befDir != 'U' && finalDirs[yidx][xidx] == 'U' {
				befDir = 'U'
				continue
			}

			if finalDirs[yidx][xidx] == 'U' && befDir == 'U' {
				finalDirs[yidx][xidx] = 'x'
			}
		}
	}
	...
	for xidx := 0; xidx < n; xidx++ {
		for yidx := 0; yidx < n; yidx++ {
			if finalDirs[yidx][xidx] != 'D' && finalDirs[yidx][xidx] != 'x' {
				befDir = finalDirs[yidx][xidx]
				continue
			}

			if befDir != 'D' && finalDirs[yidx][xidx] == 'D' {
				befDir = 'D'
				continue
			}

			if finalDirs[yidx][xidx] == 'D' && befDir == 'D' {
				finalDirs[yidx][xidx] = 'x'
			}
		}
	}
	...
	for yidx := 0; yidx < n; yidx++ {
		for xidx := 0; xidx < n; xidx++ {
			if finalDirs[yidx][xidx] != 'x' && finalDirs[yidx][xidx] != 'b' {
				res = append(res, direction{y: yidx, x: xidx, dir: finalDirs[yidx][xidx]})
			}
		}
	}
	return res
