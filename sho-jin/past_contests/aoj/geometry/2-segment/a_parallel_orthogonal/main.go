...
// ※この問題は_equalのEPSを十分に小さくしないと正しい答えにならない。
func main() {
	defer stdout.Flush()
	...
	for i := 0; i < q; i++ {
		...
		a, b, c, d := NewCoord(x0, y0), NewCoord(x1, y1), NewCoord(x2, y2), NewCoord(x3, y3)
		ab := b.minus(a)
		cd := d.minus(c)
		...
		// cos := ab.dot(cd) / (ab.norm() * cd.norm())
		// debugf("cos: %v\n", cos)

		if _equal(sin, 1.0) {
			printf("1\n")
		} else if _equal(sin, 0.0) {
			printf("2\n")
		} else {
			printf("0\n")
		}
