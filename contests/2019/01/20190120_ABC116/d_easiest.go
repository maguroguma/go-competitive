...
func main() {
	...
	// 寿司を種類情報を記憶させた上で、おいしさで降順ソートする
	sushiSlice := make(MonoList, n)
	for i := 0; i < n; i++ {
		sushi := &Mono{key: T[i], value: D[i]}
		sushiSlice[i] = sushi
	}
	sort.Sort(sort.Reverse(sushiSlice))

	// おいしさ順に愚直にK個選ぶ
	sumTaste := 0
	kinds := make(map[int]int)
	for i := 0; i < k; i++ {
		sumTaste += sushiSlice[i].value
		kinds[sushiSlice[i].key]++
	}
	kk := len(kinds)
	ans := sumTaste + kk*kk

	// K+1番目以降の寿司について入れ替えるべきかどうかをチェック
	minValueIndex := k - 1
	for i := k; i < n; i++ {
		addedK, addedD := sushiSlice[i].key, sushiSlice[i].value

		// すでにネタとして選ばれているなら、美味しさが低下するだけなのでスキップ
		if _, ok := kinds[addedK]; ok {
			continue
		}

		// minValueIndexが指す寿司について、そのネタが自身を含めて2つ以上含まれているのであれば除去する
		for kinds[sushiSlice[minValueIndex].key] <= 1 {
			minValueIndex--
			// 2つ以上選ばれている寿司がない場合、種類数をそれ以上増やすことはできず、かつおいしさの増加も見込めない
			if minValueIndex == -1 {
				fmt.Println(ans)
				return
			}
		...
		// 現在の選択寿司集合からの削除処理
		deletedK, deletedD := sushiSlice[minValueIndex].key, sushiSlice[minValueIndex].value
		kinds[deletedK]--
		minValueIndex--

		// 現在のおいしさ合計と種類数を更新
		sumTaste += -deletedD + addedD
		kk++
		kinds[addedK]++ // これを忘れると、不要に再びこの種類の寿司が選択される可能性がある
		if ans < sumTaste+kk*kk {
			ans = sumTaste + kk*kk
		}
	}
	fmt.Println(ans)
