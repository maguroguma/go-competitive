...
func main() {
	n = ReadInt()
	T, V = ReadIntSlice(n), ReadIntSlice(n)
	TT := []int{0}
	TT = append(TT, T...)
	TT = append(TT, 0)
	VV := []int{0}
	...
	for i := 0; i < len(H); i++ {
		...
		tsum := 0
		for j := 0; j < len(TT); j++ {
			l, r := float64(tsum), float64(tsum+TT[j])

			if l <= x && x <= r {
				H[i] = math.Min(H[i], float64(VV[j]))
			} else if x <= l {
				H[i] = math.Min(H[i], float64(VV[j])+1.0*math.Abs(l-x))
			} else {
				H[i] = math.Min(H[i], float64(VV[j])+1.0*math.Abs(r-x))
			}
