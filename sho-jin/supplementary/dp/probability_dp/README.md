...
Last Change: 2021-01-26 22:13:17.
...
これも期待値の線形性の感覚を身につけるには基本的な問題と言えると思う。  
ただし、実装がちょっと込み入ることになるかもしれない。  
modintの整備用とかにもいいのかも。

### [ABC144 F.Fork in the road](https://atcoder.jp/contests/abc144/tasks/abc144_f)

期待値の線形性を使いたくなるが、正解は期待値DP。

まずは `dp[i] := iからNまでの遷移回数の期待値` として後ろからDPをする。  
防ぐエッジは各ノードごとに決めることが出来て、それは明らかに `i` から遷移する先の `dp[j]` の値が最も大きいところ。  
これを各ノードについて全探索すると `O(n * m)` で解ける。  
`n = 600, m <= n(n-1)/2` なのでかなり怪しく思えるが、ループが軽いのでかなり余裕で通る。

**※ 期待値DPはたいていゴール側から求めることになりそう。**
