...
func main() {
	...
	for digit := 3; digit <= 10; digit++ {
		patterns := genPatterns(digit)
		for _, pattern := range patterns {
			if sub(pattern) {
				num, _ := strconv.Atoi(string(pattern))
				if num <= n {
					ans++
				}
			}
...
func genPatterns(digit int) [][]rune {
	return recursion(digit, []rune{})
}

func recursion(digit int, interim []rune) [][]rune {
	...
	copied := make([]rune, len(interim))
	copy(copied, interim)
	copied = append(copied, '3')
	res = append(res, recursion(digit, copied)...)

	copied = make([]rune, len(interim))
	copy(copied, interim)
	copied = append(copied, '5')
	res = append(res, recursion(digit, copied)...)

	copied = make([]rune, len(interim))
	copy(copied, interim)
	copied = append(copied, '7')
	res = append(res, recursion(digit, copied)...)
