...
func main() {
	...
	for i := 0; i < 100; i++ {
		j := i + 100
		G[i] = append(G[i], Edge{j, Weight{x}})
		G[j] = append(G[j], Edge{i, Weight{x}})
	}
	for bi := 0; bi < 99; bi++ {
		i := bi + 1
		j := bi + 100
	...
	for i := 0; i < 99; i++ {
		j := i + 1
		G[i] = append(G[i], Edge{j, Weight{y}})
		G[j] = append(G[j], Edge{i, Weight{y}})
	}
	for bi := 0; bi < 99; bi++ {
		i := bi + 100
		j := bi + 100 + 1
