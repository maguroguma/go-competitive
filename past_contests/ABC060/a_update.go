...
func main() {
	strSlice := NextStringsLine()
	a, b, c = strSlice[0], strSlice[1], strSlice[2]
	arune, brune, crune := []rune(a), []rune(b), []rune(c)
	if arune[len(arune)-1] == brune[0] && brune[len(brune)-1] == crune[0] {
		fmt.Println("YES")
	} else {
		fmt.Println("NO")
	}
