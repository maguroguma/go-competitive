...
// i は 0-based
// dp1[i+1]: i-k+1番目からi番目までを0にした場合の、i番目までの最大値
// dp2[i+1]: i番目を0にしない場合の、i番目までの最大値
...
func main() {
	...
	for i := 0; i < n; i++ {
		if i < k {
			...
			// 前者は0にする区間を1つ延長
			// 後者は新たにB[i]を含めてK個だけ後ろの薬品を0にする
			// dp2[i-k+1] がそのような場合の最大値を担保してくれる
			// 半開区間なので、dp2[i-k+1] は B[i-k] は0にせずに使っている
			dp1[i+1] = Max(dp1[i], dp2[i-k+1])

			// こちらの更新の理解は簡単
			dp2[i+1] = Max(dp2[i]+B[i], dp1[i]+B[i])
		}
	}

	fmt.Println(Max(dp1[n], dp2[n]))
}
