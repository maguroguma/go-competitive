...
func sub(start, end int, isFirst bool) int {
	if flags[start][end] {
		return dp[start][end]
	}
	...
	res := 0
	if isFirst {
		res = Max(A[start]+sub(start+1, end, false), A[end]+sub(start, end-1, false))
	} else {
		res = Min(-A[start]+sub(start+1, end, true), -A[end]+sub(start, end-1, true))
	}
	flags[start][end] = true
	dp[start][end] = res
