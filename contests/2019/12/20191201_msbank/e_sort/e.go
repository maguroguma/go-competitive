...
func (l byId) Less(i, j int) bool {
	return l.ItemList[i].id < l.ItemList[j].id
}
...
func main() {
	...
	sort.Stable(byId{L})
	// for i := 0; i < n; i++ {
	// 	fmt.Printf("id: %d, a: %d, cnt: %d, maxCnt: %d\n", L[i].id, L[i].a, L[i].cnt, L[i].maxCnt)
	// }
