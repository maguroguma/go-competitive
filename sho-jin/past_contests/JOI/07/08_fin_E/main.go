...
func main() {
	...
	for i := 0; i < n; i++ {
		x1, y1, x2, y2 := readi4()
		A[i], B[i], C[i], D[i] = x1, y1, x2-1, y2-1
	}

	for i := 0; i < n; i++ {
		a, c := A[i], C[i]
		X = append(X, a, c, max(a-1, 0), max(c-1, 0), min(a+1, w-1), min(c+1, w-1))
		b, d := B[i], D[i]
		Y = append(Y, b, d, max(b-1, 0), max(d-1, 0), min(b+1, h-1), min(d+1, h-1))
	}

	xp, xto, _ := ZaAtsu1Dim(X, 0)
	yp, yto, _ := ZaAtsu1Dim(Y, 0)
	maxX := max(xp...)
	maxY := max(yp...)
