...
func main() {
	...
	} else if str == "union_set" {
		ans = union(A, B)
	} else if str == "symmetric_diff" {
		ans = symmetricDiff(A, B)
	...
	println(PrintIntsLine(ans...))
}

func isin(a int, A []int) bool {
	for i := 0; i < len(A); i++ {
		if a == A[i] {
			return true
		}
...
func subtractX(A []int, x int) []int {
	ma := make(map[int]int)
	for _, a := range A {
		ma[a] = 1
	}
