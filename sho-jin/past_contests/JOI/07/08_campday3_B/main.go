...
type NumberPQ []*Number

// Interfaces
func NewNumberPQ() *NumberPQ {
	temp := make(NumberPQ, 0)
	pq := &temp
	heap.Init(pq)

	return pq
}
func (pq *NumberPQ) push(target *Number) {
	heap.Push(pq, target)
}
func (pq *NumberPQ) pop() *Number {
	return heap.Pop(pq).(*Number)
}

func (pq NumberPQ) Len() int { return len(pq) }
func (pq NumberPQ) Less(i, j int) bool {
	L, R := pq[i], pq[j]
	return L.nu*R.de < L.de*R.nu
} // <: ASC, >: DESC
func (pq NumberPQ) Swap(i, j int) {
	pq[i], pq[j] = pq[j], pq[i]
}
func (pq *NumberPQ) Push(x interface{}) {
	item := x.(*Number)
	*pq = append(*pq, item)
}
func (pq *NumberPQ) Pop() interface{} {
	old := *pq
	n := len(old)
	item := old[n-1]
	*pq = old[0 : n-1]
