...
func main() {
	...
	for i := 0; i < m; i++ {
		x, y := X[i], Y[i]
		nextNodes[x] = append(nextNodes[x], y)
	...
	for i := 1; i <= n; i++ {
		if memo[i] == 0 {
			rootNodeList = append(rootNodeList, i)
			// dp[i] = 0
		}
	}

	ans := -1
	for _, rootNodeId := range rootNodeList {
		ChMax(&ans, dfs(rootNodeId))
	}

	fmt.Println(ans)
}

func dfs(nodeId int) int {
	if dp[nodeId] != -1 {
		return dp[nodeId]
	}

	res := -1
	for _, nid := range nextNodes[nodeId] {
		ChMax(&res, dfs(nid)+1)
	}
