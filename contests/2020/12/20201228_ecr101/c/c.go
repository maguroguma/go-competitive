...
func solve() {
	...
	for pq.Len() > 0 {
		...
		if H[right]-H[mid] > bo {
			printf("NO\n")
			return
		} else if H[right]-H[mid] >= 0 && H[right]-H[mid] <= bo {
			diff := H[right] - H[mid]
			if diff > k-1 {
				// H[mid] += diff - (k - 1)
				chmax(&D, diff-(k-1))
		...
		if H[left]-H[mid] > bo {
			printf("NO\n")
			return
		} else if H[left]-H[mid] >= 0 && H[left]-H[mid] <= bo {
			diff := H[left] - H[mid]
			if diff > k-1 {
				// H[mid] += diff - (k - 1)
				chmax(&D, diff-(k-1))
	...
	// 両端を個別にチェック
	// if abs(H[0]-H[1]) > bo || abs(H[n-1]-H[n-2]) > bo {
	// 	printf("NO\n")
	// 	return
	// }
	if abs(H[0]-H[1]) > k-1 || abs(H[n-1]-H[n-2]) > k-1 {
		printf("NO\n")
		return
	}
