...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		S[i], T[i], X[i] = s, t, x
		L = append(L, &Item{key: x, s: s, t: t, x: x})
	}
	D = ReadIntSlice(q)

	sort.Stable(L)
	// for i := 0; i < n; i++ {
	// 	fmt.Printf("s: %d, t: %d, x: %d\n", L[i].s, L[i].t, L[i].x)
	// }
...
OUT_FOR:
	for i := 0; i < n; i++ {
		item := L[i]
		s, t := item.s-item.x, item.t-item.x

		l, r := getLeft(s), getRight(t)
		for j := l; j <= r; j++ {
			if counter == q {
				break OUT_FOR
			}
...
func getLeft(s int) int {
	isOK := func(idx int) bool {
		if D[idx] >= s {
			return true
		}
		return false
