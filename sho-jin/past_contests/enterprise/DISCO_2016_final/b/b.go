...
func main() {
	...
	for i := 0; i < n; i++ {
		t, a := T[i], A[i]
		L = append(L, &Food{key: t, val: a, dead: t})
	}
	sort.Stable(byKey{L})

	const maxi = 100000 + 50
...
func C(mid int) bool {
	...
	for i := 0; i < len(L); i++ {
		if curTime+1 <= L[i].dead && curTime+1 <= mid {
			// 取れるなら取る
			sum += L[i].val
			curTime++
			heap.Push(pq, L[i].val)
		} else if pq.Len() > 0 {
			// 現在取っているものの最小と比較して取るか決める
			mini := heap.Pop(pq).(int)
			if mini < L[i].val {
				sum -= mini
				sum += L[i].val
				heap.Push(pq, L[i].val)
