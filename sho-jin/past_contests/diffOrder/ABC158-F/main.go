...
func main() {
	...
	// })
	// for i := 0; i < n; i++ {
	// 	_, curR := segments[i][0], segments[i][1]
	...
	m := func(a, b S) S { return S(max(int(a), int(b))) }
	st := NewSegtree(v, e, m)
	sort.Slice(segments, func(i, j int) bool {
		return segments[i][0] < segments[j][0]
	})
	...
	for i := n - 2; i >= 0; i-- {
		...
		ok := BinarySearch(i, n, func(m int) bool {
			return right > segments[m][0]
		})
...
func MergeSegments(srcSegments [][2]int) [][2]int {
	...
	if isInitialized {
		...
		for j < len(srcSegments) {
			T[j] = len(srcSegments) - 1
			j++
		}
	}
