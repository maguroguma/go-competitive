...
func main() {
	...
	maxA, minA := -INF_B60, INF_B60
	maxAIdx, minAIdx := -1, -1
	maxB, minB := -INF_B60, INF_B60
	maxBIdx, minBIdx := -1, -1
	for i := 0; i < n; i++ {
		a, b := A[i], B[i]
		if chmax(&maxA, a) {
	...
	debugf("%d %d %d %d\n", maxAIdx, minAIdx, maxBIdx, minBIdx)

	// 絞り込んで全探索！
	D := max(maxA-minA, maxB-minB)
	PA := []int{maxA - D/2, minA + D/2}
	PB := []int{maxB - D/2, minB + D/2}
	for _, pa := range PA {
		for _, pb := range PB {
			x := (pa + pb) / 2
			y := (pb - pa) / 2
	...
	// 短い方を調整（※短いほうを形成する点が正方形の周上に乗らないのでこの方法は的外れ）
	// a := (maxA + minA) / 2
	// b := (maxB + minB) / 2
...
func check(cx, cy int) bool {
	manD := abs(cx-X[0]) + abs(cy-Y[0])
	for i := 1; i < n; i++ {
		x, y := X[i], Y[i]
		d := abs(cx-x) + abs(cy-y)
		if manD != d {
