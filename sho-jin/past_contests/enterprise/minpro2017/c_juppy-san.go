...
func main() {
	...
	// 検索したいAの文字列の取得は、Sをソートする前に行う
	astrs := []string{}
	for _, a := range A {
		a--
		astrs = append(astrs, S[a])
	}
	...
	ans := ""
	exist := false
	// 毎回zの列を作る必要はないので事前に準備しておく
	zString := strings.Repeat("z", PowInt(10, 4)+1) // 本当は10^5じゃないとダメだが、4乗じゃないとTLEしてしまった
	for i := 0; ; i++ {
		...
		if astrs[0][i] == astrs[len(astrs)-1][i] {
			keyString := astrs[0][:i+1]
			num := lb(S, keyString+zString) - lb(S, keyString)
			if num == k {
				ans = keyString
				exist = true
				break
...
func lb(s []string, key string) int {
	isOK := func(index int, key string) bool {
		if s[index] >= key {
			return true
		}
		return false
