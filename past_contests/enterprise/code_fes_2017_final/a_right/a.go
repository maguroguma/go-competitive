...
var dists [100000+5]map[int]int

func main() {
	n, m = ReadInt(), ReadInt()
	A, B, L = make([]int, m), make([]int, m), make([]int, m)
	for i := 0; i < m; i++ {
		A[i], B[i], L[i] = ReadInt()-1, ReadInt()-1, ReadInt()
	}

	for i := 0; i < n; i++ {
		dists[i] = make(map[int]int)
	}
	...
	for i := 0; i < n; i++ {
		tmpAns := 0
		for dist, num := range dists[i] {
			if dist == 1270 {
				continue
			}
			tmpAns += num * dists[i][2540 - dist]
		}
		ans += tmpAns/2

		if dists[i][1270] > 0 {
			ans += dists[i][1270] * (dists[i][1270] - 1) / 2
		}
	}
