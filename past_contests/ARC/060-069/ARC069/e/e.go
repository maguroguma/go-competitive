...
func (l byKey) Less(i, j int) bool {
	...
	if l.ItemList[i].key > l.ItemList[j].key {
		return true
	} else if l.ItemList[i].key < l.ItemList[j].key {
		return false
	}
	return l.ItemList[i].idx < l.ItemList[j].idx
}
...
func main() {
	...
	L := make(ItemList, 0, 200000+5)
	for i := 0; i < n; i++ {
		L = append(L, &Item{key: A[i], idx: i})
	}
	sort.Stable(byKey{L})
	// for i := 0; i < n; i++ {
	// 	PrintDebug("v: %d, pos: %d\n", L[i].key, L[i].idx)
	// }
	...
	for i < n {
		...
		// 差分だけminsのpositionに足して次へ
		pos := mins[i]
		curVal := V[i]
		nextVal := V[i+1]
		answers[pos] += (curVal - nextVal) * (i + 1)
