...
var n, m int
var A []int
var lm, rm [100000+5]map[int]bool
...
func main() {
	...
	for i := 0; i < n; i++ {
		lm[i], rm[i] = make(map[int]bool), make(map[int]bool)
	}

	sort.Sort(sort.Reverse(sort.IntSlice(A)))

	ans := 0
	temp := make(ItemPQ, 0, 100000)
	pq := &temp
	heap.Init(pq)
	heap.Push(pq, &Item{pri: A[0]+A[0], left: 0, right: 0})
	lm[0][0] = true
	for m > 0 {
		p := heap.Pop(pq).(*Item)
		ans += p.pri
		...
		if l+1 < n {
			if _, ok := lm[l+1][r]; !ok {
				heap.Push(pq, &Item{pri: A[l+1]+A[r], left: l+1, right: r})
				lm[l+1][r] = true
			}
		}
		if r+1 < n {
			if _, ok := lm[l][r+1]; !ok {
				heap.Push(pq, &Item{pri: A[l]+A[r+1], left: l, right: r+1})
				lm[l][r+1] = true
			}
		}
	}
