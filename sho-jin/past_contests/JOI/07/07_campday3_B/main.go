...
func dd(x1, y1, x2, y2 int) int {
	return (x2-x1)*(x2-x1) + (y2-y1)*(y2-y1)
}

func toVid(pid, cid int) int {
	return pid*n + cid
}
func pbid(vid int) int {
	return vid / n
}
func bid(vid int) int {
	return vid % n
}
...
func main() {
	...
	transit := func(cv *Vertex, AG [][]Edge) []*Vertex {
		...
		for _, e := range AG[j] {
			...
			nvid := toVid(j, k)
			nv := &Vertex{nvid, Value{cv.v.v + e.w.v}}
	...
	N := n * n
	SS := []StartPoint{{toVid(0, 0), Value{0}}}
