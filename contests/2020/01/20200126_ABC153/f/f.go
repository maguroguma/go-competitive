...
func (l MonsterList) Len() int {
	return len(l)
}
func (l MonsterList) Swap(i, j int) {
	l[i], l[j] = l[j], l[i]
}

func (l byKey) Less(i, j int) bool {
	return l.MonsterList[i].key < l.MonsterList[j].key
}

// how to use
// L := make(MonsterList, 0, 200000+5)
// L = append(L, &Monster{key: intValue})
...
func main() {
	...
	L = make(MonsterList, 0)
	for i := 0; i < n; i++ {
		L = append(L, &Monster{key: X[i], x: X[i], h: H[i]})
	}
	sort.Stable(byKey{L})
	...
	ti := 1 << 40
	ei := 0
	lst := NewLazySegmentTree(n, f, g, h, p, T(ti), E(ei))
	for i := 0; i < n; i++ {
		lst.Set(i, T(L[i].h))
	}
	lst.Build()
	...
	for i := 0; i < n; i++ {
		...
		num := (monH + (a - 1)) / a
		ans += num
		// x+2d以下の座標のモンスターで最も右に居るものを探索
		mri := sub(x + 2*d)
		// [x, x+2*d]に対してRAQ
		lst.Update(i, mri+1, E(-a*num))
	}
